...
// DM path: PFC choke ripple spectrum (triangular, 50 kHz, worst-θ 21.4 App per phase) → the drawn per-phase DM ladder
// (3 star X2 stages + 2 CM-choke leakages) → LISN 50 Ω.
// CM path: node dv/dt (DPT: 46 V/ns eff.) × heatsink capacitance 200 pF spectrum → the drawn two-stage ladder
// (Y trio at AC1..3 and at AC1M..3M, CM chokes 2 mH each, nanocrystalline µ(f)) — gated ≥ +3 dB like DM.
...
// The DM path is solved as the DRAWN per-phase ladder, like the CM path — current injected at the converter node, divided
// stage by stage toward a 50 Ω per-line LISN. Star X caps enter at their own value; a Δ cap enters as its per-phase star
// equivalent 3·C. Drawn: LF─CX0 4.7 µF★─CMC1 L_lk─CX1 4.7 µF★─CMC2 L_lk─CX2 4.7 µF★─PFC, with no DM choke.
// The second ladder below is the REFERENCE the gate compares against (CMC1 L_lk─CX1 2.2 µF Δ─CMC2 L_lk + a DM crest
// choke─CX2 4.7 µF Δ): that DM choke is NOT fitted, so the drawn ladder has to attenuate at least as well without it.
...
const attDM = (fHz) => dmLadder(fHz, DM_E68(9e-6));   // 30 kW ripple through the drawn filter at mid leakage
// CM: trapezoid dv/dt spectrum: Vn ≈ 2·Vbus/(π n)·sinc-ish with corner at 1/(π·tr); i_cm = Vn·ω·Cp
const TR = 9e-9, CP = 200e-12, VSW = 425;
// The LLC bridge is a SECOND CM source on the same extrusion — two leg nodes swinging the full 830 V bus at the PFM
...
// ≤ 50 pF. DESIGN REQUIREMENT (DFM + T-39 STOP): LLC leg-node-to-PE capacitance ≤ 50 pF in total — a shielded thermal interface
...
// fundamental out of the band, but the tank scan reads +20 % rms at the 764 V corner, so it is not used. CP_LLC below is the requirement.
...
// The CM path is solved as the DRAWN ladder, not a squared 30 kHz per-stage corner. Converter-side
// Norton source → Y trio on AC1..3 (CY1-3) → CMC2 (+LDM/3) → Y trio on AC1M..3M (CY4-6) → CMC1 → LISN
// (3 lines in parallel, 50/3 Ω). verify-independent proves the netlist carries exactly this ladder. The
// 1709611 reference board carries only the AC1..3 trio — CMC1+CMC2 4 mH in series against 14.1 nF, one stage.
...
// The converter-side trio CY1-3 is 10 nF (Y2 class) so the LLC bridge's in-band fundamental has margin: with the LLC source
// counted a 4.7 nF trio reads +0.2 dB at a 50 pF leg-node capacitance, 10 nF reads +7.1 dB (50 pF) / +3.1 dB (100 pF).
...
const CM_DRAWN = [{ L: LCM, C: CY3 }, { L: LCM, C: CY3_CONV }];   // LISN←CMC1←CY4-6 (3 × 4.7 nF)←CMC2←CY1-3 (3 × 10 nF) (LDM/3 ≈ 4 µH in the CM path: < 0.3 %, dropped)
const CM_1709611 = [{ L: 2 * LCM, C: CY3 }];                  // control: the single-trio board
const attCM = (fHz, st = CM_DRAWN) => ladder(fHz, st);

const rows = [["f_kHz", "DM_dBuV", "CM_dBuV", "limitA_dBuV", "margin_worst_dB"]];
const series = { dm: [], cm: [], lim: [] };
let worstMargin = 1e9, worstAt = 0;
for (let n = 3; n * FSW <= 30e6; n++) {
  const fHz = n * FSW;
  if (fHz < 150e3) continue;
  const dm = dbuv(Ih(n) * attDM(fHz) * 25);                              // one lane per module; LISN transimpedance ≈ 25 Ω
  const nCorner = 1 / (Math.PI * TR);
  const vn = (2 * VSW) / (Math.PI * n) * (fHz < nCorner ? 1 : nCorner / fHz);
  const icm = (vn * CP + vnLlc(fHz) * CP_LLC) * 2 * Math.PI * fHz * attCM(fHz);   // Vienna + LLC CM sources
  const cm = dbuv(icm * 25);
  const lim = limitA(fHz);
  const worst = Math.max(dm, cm);
  const margin = lim - worst;
  if (margin < worstMargin) { worstMargin = margin; worstAt = fHz; }
  if (n % 2 === 1 || n % 10 === 0) {
    rows.push([f(fHz / 1e3, 0), f(dm), f(cm), lim, f(margin)]);
    series.dm.push({ x: fHz, y: dm });
...
plotSVG({ title: "Conducted pre-compliance estimate vs CISPR-32 A (±20 dB band applies)", xlabel: "Hz", ylabel: "dBµV",
  ...
  series: [mk("dm", "DM (30 kW)"), mk("cm", "CM est.", "#A83232"), mk("lim", "Class A QP", "#666")] });
console.log(`Worst margin: ${f(worstMargin)} dB at ${f(worstAt / 1e3, 0)} kHz (positive = under limit).`);
// ---- Per-MODULE-VARIANT DM margins: each variant's own ripple source (dIpp) through the drawn ladder at both CM-leakage
// band edges, against the reference filter with a DM crest choke on the same model. Gate: ≥ +3 dB AND no worse than that
// reference — dropping the DM choke may not cost attenuation.
{
  ...
  const worst = (dipp, net) => { let wm = 1e9, wa = 0;
    ...
    return [wm, wa]; };
  console.log("Per-variant DM worst margins (drawn per-phase ladder, CM leakage band 6–12 µH):");
  for (const [sku, dipp] of Object.entries(VAR)) {
    ...
    console.log(`  ${sku}: 12× X2 4.7 µF (3 star stages, 2 per phase at the converter), no DM choke → worst DM margin ${f(m6)} dB at ${f(a6 / 1e3, 0)} kHz (L_lk 6 µH) · ${f(m12)} dB (12 µH) · [control: the same ladder with a ${D6[sku].Lpk} µH crest DM choke] ${f(mc)} dB`);
    if (m6 < 3 || m6 < mc) { console.log(`  ${sku}: DM MARGIN UNDER +3 dB OR BELOW THE DM-CHOKE CONTROL — the drawn filter is insufficient`); process.exitCode = 1; }
  }
}
// ---- CM margin gate on the drawn two-stage ladder
{
  const cmWorst = (cp, st, cpLlc = CP_LLC * (cp / CP)) => {
    ...
    for (let n = 3; n * FSW <= 30e6; n++) {
      ...
      const vn = (2 * VSW) / (Math.PI * n) * (fHz < nCorner ? 1 : nCorner / fHz);
      const m = limitA(fHz) - dbuv((vn * cp + vnLlc(fHz) * cpLlc) * 2 * Math.PI * fHz * attCM(fHz, st) * 25);   // Vienna + LLC sources
      if (m < wm) { wm = m; wa = fHz; }
    }
    return [wm, wa];
  };
  ...
  console.log(`CM (drawn ladder CY1-3 · CMC2 · CY4-6 · CMC1, µ(f) roll-off ${f(muNano(150e3) / muNano(10e3), 2)} at 150 kHz; sources Vienna 425 V @ ${CP * 1e12} pF + LLC 830 V @ ${CP_LLC * 1e12} pF): worst margin ${f(m0)} dB at ${f(a0 / 1e3, 0)} kHz @ Cp ${CP * 1e12} pF`
    + ` · ${[400e-12, 600e-12].map((c) => `${c * 1e12} pF ${f(cmWorst(c, CM_DRAWN)[0])} dB`).join(" · ")} · Cp budget for +3 dB = ${f(cpMax * 1e12, 0)} pF switch-node→PE`);
  console.log(`  LLC leg-node capacitance requirement ≤ ${CP_LLC * 1e12} pF (shielded pad): the LLC fundamental (528 V envelope at 150–203 kHz) alone reads ${[50e-12, 100e-12, 200e-12].map((c) => `${c * 1e12} pF → ${f(cmWorst(CP, CM_DRAWN, c)[0])} dB`).join(" · ")} — T-39 measures it; STOP below +3 dB`);
  console.log(`  [1709611 control] one Y trio (CMC1+CMC2 = 4 mH vs 14.1 nF): worst CM margin ${f(mc)} dB — a squared-corner model credits it +22 dB for a second stage that is not drawn`);
