...
export const MOUNT = {
  air: { rth: 0.8, ref: 70 },      // K/W junction → extrusion base under the die · °C base reference at 55 °C ambient (pre-E81 basis, kept for the record)
  liquid: { rth: 0.65, ref: 65 },  // K/W junction → coldplate · °C plate reference (E42)
};
// E81 (review F-C-2): a base at 70 °C is unreachable when the module's own air rise is 12–18 K at 55 °C inlet — the air LEAVING the
// extrusions is 67–73 °C, and a sink cannot be colder than its outlet air. Interim per-SKU base references from the air-side energy
// balance (ρ at 55 °C, fan flow × 0.6, ≈55 % of the flow over the DC-DC face, ε–NTU on a 3U extrusion): 30 kW 74 °C (three fans,
// E81), 40 kW 75 °C, 50 kW air 77 °C. EVT T-04 measures the base under the die at inlet, mid and outlet and replaces these; the
