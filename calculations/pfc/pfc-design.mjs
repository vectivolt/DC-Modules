...
import { d1Loss } from "../magnetics/d1-choke.mjs";
  ...
  for (const g of GEOMS)
    for (const mat of MATS)
      for (let stack = 1; stack <= 5; stack++)   /* E41: taller stacks of the SAME real core before any new p/n */
        for (const nw of STRANDS) {
          const aw = (nw * Math.PI * 1.6e-3 ** 2) / 4;
          ...
          for (let N = 10; N <= 80; N++) {
            ...
            let wound; try { wound = toroidWound(g.core, stack, { N, nw, d: 1.6e-3 }); } catch { break; }   // bore closed: no more turns fit
            const MLT = wound.mlt;
            const cu = d1Loss({ N, nw, d: 1.6e-3, stack, lay: 1 }, { I1pk_A: Ipk, Ihf_rms_A: Math.sqrt(dI2), Pfe_igse_W: 0 }, 100);
            const Rdc = cu.Rdc, Pcu = cu.lf + cu.hf;
            const Pfe = pvMAS(mat.mu, fsw, dBmax / 2) * g.Vc * 1e-6 * stack;
            const P = Pcu + Pfe;
            const dT = Math.pow((P * 1000) / ((wound.area.outer + wound.area.face + wound.area.bore) * 1e4), 0.833);
            if (dT > 65) continue;
