...
ck("TRIP-GUARD-AFTER-ENABLE", (portC.match(/trip_guard\(\);/g) || []).length === 2 && /hrtimer_pfc_apply\(&po\);\n  trip_guard\(\);/.test(portC) && /hrtimer_llc_apply\(&lo\);\n  trip_guard\(\);/.test(portC)
  && (hrtC.match(/HRT_CHOUTEN = /g) || []).length === 2 && !/hrtimer_rearm|fault_rearm/.test(portC + hrtC + readFileSync(join(ROOT, "firmware/hal/app.h"), "utf8"))
  && /bool hrtimer_trip_pending\(void\) \{ return \(HRT_INTF & \(0x1Fu \| BIT\(6\)\)\) != 0u; \}/.test(hrtC) && (hrtC.match(/HRT_INTC = /g) || []).length === 2,
  "the two control interrupts are the ONLY writers that can enable a power output, and after each such write the port tests the software trip count AND the HRTIMER fault flags, disabling everything if either moved — a fault landing between the decision and the CHOUTEN write otherwise re-arms a killed stage; no tick-side re-arm exists, and only the fault interrupt (and the DLL calibration wait at init) writes the flag-clear register");
