// bom-gen.mjs — BOM generator: reads the built circuit JSON of every board, classifies each
// component through the parts-db patterns, and emits the per-module BOM CSVs, the costed roll-ups
// at 100/1k/5k/10k pcs with second sources, docs/bom-cost.md and one page per module SKU.
// The 10k tier is the planning basis (≥10k units/yr): ×0.80 electronics / ×0.87 mechanical off the
// 1k basis, overridden per part wherever a volume-specific quote exists (`p10k` in parts-db).
// UNMATCHED components are listed loudly — the BOM is not complete until that list is empty.
// Run (after tsci builds): node calculations/cost/bom-gen.mjs

import { readFileSync, writeFileSync, existsSync, readdirSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
import { DB, skuOverrides, mechLines, mech2U, BUILDABLE_SKUS } from "./parts-db.mjs";
import { lcscFor, lcscForPart, lcscSummary, LCSC_BY_VALUE } from "./lcsc-map.mjs";
import { realPackagesFrom } from "../footprint-map.mjs";   // the drawn land decides the part
import { footer, masthead } from "../doc-chrome.mjs";
const ROOT = join(dirname(fileURLToPath(import.meta.url)), "..", "..");
const f = (x, d = 0) => Number(x.toFixed(d));

const SKUS = BUILDABLE_SKUS; // the four module SKUs: 30 kW · 40 kW · 50 kW liquid · 50 kW air
const TARGETS = { "40kw": [33000, 29000],  // 30k red-line ×1.33 rounded — provisional until a pricing directive
   "50kw": [43000, 39000],                 // 40k red-line ×1.25 + coldplate adder — provisional until a pricing directive
   "50kwa": [43000, 39000],                // air twin, same provisional line
   "30kw": [25000, 22000] };
...
const CAT = (mpn, desc) =>
  /SiC|MOSFET|JBS|FET 1200|650 V 4 A/.test(desc) ? "semiconductors"
    ...
    : /choke|transformer|inductor|trim/i.test(desc) || /\bCT\b/.test(desc) ? "magnetics"   // \bCT\b stays case-SENSITIVE: a case-insensitive "ct" matches every relay "contact"
...
// China-supplier RFQ-TARGET basis: landed factors (freight + ~10 % duty included) on the India 10k estimate, by part category.
// FLAGGED TARGETS, not quotes — they state the price a China RFQ must reach for the parity comparison.
...
// Every designator's schematic section, read off the sheet payloads, so a per-SKU BOM page groups cost the way the release
// sheets are drawn (sheet-pages → sheet-netlist-gen → kicad5). Missing payloads leave parts under "Unassigned".
const SECTION_TITLE = {
...
const summary = {};
const detail = {};   // per-SKU section roll-up and line items for docs/bom-<sku>.md
...
for (const sku of SKUS) {
  ...
  // ONE control card per module, carrying both converter roles — its parts are costed here.
  for (const [side, mult, pth] of [["acdc", 1, null], ["dcdc", 1, null], ["card", 1, join(ROOT, "dist", "boards", "control-card", "circuit.json")]]) {
    ...
    const landOf = realPackagesFrom([p]);        // chip size per designator, read off the built land
    for (const c of j.filter(e => e.type === "source_component")) {
      ...
      const val = valueOf(c);
      const pkg = landOf.get(name);              // one family|value can be two parts by land
  ...
  for (const r of rows) {
    ...
    if (!lc.status) throw new Error(`bom-gen: ${r.mpn} has no lcsc_status`);   // a blank sourcing state must never reach a BOM line
  ...
  const m2 = mech2U[sku] ? mech2U[sku].reduce((a, [, q, pr]) => a + q * pr, 0) : null;   // 2U scenario: mechanical/assembly lines only
...
// ---- price per kW: ₹ / kW must fall 30 → 40 → 50 kW, with both 50 kW twins below the 40 kW module ----
const PKW = [
...
// ---- one BOM page per module SKU (docs/bom-<sku>.md) — generated, never hand-edited ----
...
| **China RFQ target @10k** | **₹${inr(s.g10kCN)}** · ₹${inr(s.g10kCN / kw)} / kW — landed targets, not quotes |
${s.s2U !== null ? `| **2U construction scenario** | ₹${inr(s.s2U)} · China target ₹${inr(s.s2UCN)} — flagged estimate, not the design basis |\n` : ""}| **1k · 5k · 100 pcs** | ₹${inr(s.grand)} · ₹${inr(s.g5k)} · ₹${inr(s.g100)} |
