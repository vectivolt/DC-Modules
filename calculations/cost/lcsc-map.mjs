...
export const LCSC = {
  ...
  "SIC-1700-1R":       { lcsc: "C5713500", mpn: "C2M1000170D",   status: "REVIEW", note: "SUPERSEDED AT E82 (M-04) by SIC-1700-1R-G15 — DO NOT ORDER for QAUX: the C2M1000170D 1 Ω is a V_GS = 20 V figure and the NCP1252 DRV pin clamps at 15 V typ (Rev 9), so this die is never fully enhanced on this driver. No live parts-db rule names this row" },
  ...
  // ---- E81 (review E81): every part this review added or re-classed. Nothing here is guessed —
  // a CLASS row states the rating purchasing buys against, a REVIEW row names the open decision.
  "ELH-470u500":       { status: "CLASS", spec: "470 µF **500 V** snap-in, 105 °C, DC-link grade, −40 °C category (A11 rev C −30 °C floor). E81 (F-C-6): 450 V ran 92 % nominal / 97.8 % at F.38, and the free bus-side alternatives are closed (810 V fails the 500 V-bank FHA gain; capping LOW at 480 V pushes 480–500 V into HIGH with a 240–250 V bank). Binding RFQ line: height ≤ the drawn can, ripple ≥ 3 A rms @100 kHz/105 °C, ESR ≤ 0.15 Ω, leakage ≤ 0.5 mA/can at 85 °C/415 V graded per half, endurance lot 1000 h at 438 V/105 °C ×5" },
  "PP-2u2-1100":       { status: "CLASS", spec: "2.2 µF 1100 V PP film, 27.5 mm pitch — the capacitor of the E81 DC-link entry RC damper (F-G-1 FIX-D)" },
  "RTF-50W-R33-TO247": { status: "CLASS", spec: "0.33 Ω ≥ 50 W thick-film NON-INDUCTIVE, TO-247 two-lead, heatsink-clipped — the resistor of the DC-link entry RC damper; E82 (F-H1-7): 7 / 16 / 36 W worst per SKU whenever the bridge runs at f_max (2·f_sw on the entry-film / stud resonance)" },
  "CER-3W-R33-AX":     { status: "SUPERSEDED", spec: "E82 (F-H1-7): the ≥3 W axial rested on a 1.8 W figure; the damper takes 7–36 W — replaced by RTF-50W-R33-TO247" },
  ...
  "C1206-330p-1kV-C0G":{ status: "CLASS", spec: "330 pF 1 kV C0G 1206 — LLC per-die turn-off snubber at 30 kW (tanks.mjs cs; the E81 weak-leg close-out: 470 pF and above shut the leg-A ZVS window at the SER250-bus764 corner). E82: the BOM had carried the pre-close-out 1 nF part number against a 330 pF drawing" },
  "C1206-680p-1kV-C0G":{ status: "CLASS", spec: "680 pF 1 kV C0G 1206 — LLC per-die turn-off snubber at 40 kW (tanks.mjs cs; the knee — 1 nF rings that SKU's PAR500 corner to 91 % of 1200 V)" },
  "C1812-330p-1k":     { status: "CLASS", spec: "330 pF 1 kV C0G 1812 — Vienna snubber (E81 F-C-3/F-C-13: 100 pF as drawn left the unclamped half-cycle at 85–89 % of 750 V on the 50s)" },
  "MLCC-18p-0603":     { status: "CLASS", spec: "18 pF 0603 C0G/NP0 50 V — LLC DESAT blank (E81 F-C-8 close-out: two dies per channel double the soft-off charge; 22 pF sat over 75 % of the SCWT, 10 / 15 pF under the 0.4 µs noise floor)" },
  "R2512-22k-HV-AS":   { status: "CLASS", spec: "22 kΩ 2512 3 W anti-surge HV, Umax ≥ 250 V, 2-series per half — the E81 DC-link balance value", note: "SUPERSEDED AT E82 (M-10): back to 47 k and ONE string per half per module. E81 sized against a 17 mA leakage imbalance built from the datasheet LIMIT; the RFQ line E81 itself added (≤ 0.5 mA/can hot, graded per half) caps the realistic imbalance at ≈ 1.2 mA. No live parts-db rule names this row" },
  "R2512-4k7-1W":      { status: "CLASS", spec: "4.7 kΩ 1 W 2512 — V24 preload (E81 F-A-10: with no minimum load V24 peak-charged to 25–26.5 V against a 26.4 V relay-coil maximum)" },
  "X1-4u7-530":        { status: "CLASS", spec: "X1 4.7 µF 530 VAC film — 50 kW input-filter damper cap (E81 F-G-5)" },
  ...
  "Y2-10n-300":        { status: "CLASS", spec: "Y2 10 nF 300 VAC safety film — converter-node L-PE trio (E81 F-L-4). NOTE the coupled decision: PE leakage scales with C, so this takes a module from 1.07 to 1.67 mA at 1.1 × 475 VAC with one line open" },
  "GDT-2k5-20kA":      { status: "CLASS", spec: "gas discharge tube 2.5 kV, ≥20 kA 8/20 µs, 8 mm — L-PE surge path in series with the S20K550 (E81 F-A-18: a 3.5 kV GDT never fires at the OVC III 4 kV level)" },
  "BZX84-B15":         { status: "ORDERABLE", mpn: "BZX84-B15", note: "15 V ±2 % zener SOT-23 — E81 (F-A-10): the ±5 % BZT52-C15 put V15's upper corner at 16.6 V, outside the bias modules' 13.5–16.5 V window" },
  "SMBJ18A":           { status: "ORDERABLE", note: "V15 rail clamp — E81 (F-A-10): SMBJ16A's standoff sat below the rail's normal maximum, i.e. a heater, not a clamp" },
  "SMBJ28A":           { status: "ORDERABLE", note: "V24 rail clamp — E81 (F-A-10), paired with the new 4.7 k preload" },
  "XTAL-8M-30PPM-3225":{ status: "CLASS", spec: "8 MHz ±30 ppm (incl. tempco + ageing over −40…+105 °C) SMD crystal, 3.2 × 2.5 mm, CL 12 pF, ESR ≤ 150 Ω — E81 (F-A-30/F-F-1): CAN off the ±2–3 % IRC8M is 5× the ISO 11898-1 budget" },
  "TP-1mm-PAD":        { status: "CLASS", spec: "1 mm bare test pad on WDI — fixture kicks the watchdog while the WDO→NRST link is lifted for blank-chip SWD programming (E81 F-F-8); a fab-drawing item, not a purchased part" },
  "QA01C-15":          { status: "REVIEW", mpn: "QA01C-15", note: "SUPERSEDED AT E82 (M-03) by the CLASS row ISO-GBIAS-15-1W — this order code does not exist in MORNSUN's catalogue and no live parts-db rule names it any more. Historical text: MORNSUN **+15/−3 V** 1 W gate-bias module — E81 (F-C-15): the registered second source is −4/+15 V STATIC with a +19 V transient absolute maximum, which the drawn +18 V plus a real gate loop's overshoot crosses. Same price class as the -18 variant. REVIEW = confirm the exact order code and the RFQ bulk-decoupling per the MORNSUN app note; C_io ≤ 5 pF for the F-C-24 CM budget" },
  "QA02C-15":          { status: "REVIEW", mpn: "QA02C-15", note: "SUPERSEDED AT E82 (M-03) by the CLASS row ISO-GBIAS-15-2W — same reason. Historical text: MORNSUN **+15/−3 V, 2 W** gate-bias module for the LLC channels that drive TWO dies (40/50 kW) — E81 (F-C-23): P_gate + driver I_CC2 reaches 1.59 W at f_max 203 kHz, 159 % of a 1 W module, and a droop hits the NSI66x1A 9.8–11.8 V VCC2 UVLO. REVIEW = order code + the same C_io line" },
  "HFE18V-250A-AUX":   { status: "REVIEW", mpn: "HFE18V/NB90 or TE EV200 class", note: "E81 (F-H-8): the 50 kW 'HF167F-250A-M' was not a real part — HF167F tops at ~100 A main. A genuine 250 A contactor with a 1 Form A auxiliary is needed: make ≥ 360 A pk (E73 bypass-closure inrush), ≤ 70 V across the contacts, ≥ 30 000 makes, and its own land pattern re-checked before BOM freeze. Price is an estimate until the RFQ lands" },

  // ---- E82 (independent validation review): every part this round added or re-classed -----------
  "R2512-33k-HV-AS":   { status: "CLASS", spec: "33 kΩ 2512 2 W anti-surge HV, U_max ≥ 250 V, 2-series per position — the artificial star AND the X-capacitor bleed path (E82 M-12: at 47 k the X terminals took 5.3 / 6.4 s to 60 V after E68's third X stage, over the 5 s line for permanently connected equipment)", note: "same UNI-ROYAL PS / KOA anti-surge 2512 family as PS122WF4702T4E (R2512-47k-HV-AS) — value row at read-back, never invented" },
  "R2512-150k-HV":     { status: "CLASS", spec: "150 kΩ 1 % 2512 HV, U_max ≥ 400 V, 3-series across the output studs — passive output bleeder behind DOUT (E82 M-11): 450 kΩ on 9.4 µF = τ 4.2 s, < 60 V in ≈ 12 s from 1000 V; 0.74 W / 333 V per element", note: "HV-rated 2512 families (KOA HV73 tops out at 1206, so this is the RV2512 / UNI-ROYAL PS-HV class) — C-number to be read back" },
  "R2512-10R-3W-PULSE":{ status: "CLASS", spec: "10 Ω 2512 **3 W pulse-proof** — Vienna RC snubber (E82 M-07: C_SN 100 → 330 pF at E81 took the dissipation to 2.84 W, 142 % of the 2 W part the E28 '0.86 W' text justified)", note: "pulse-rated thick film (Bourns CRM / Vishay PLT / KOA SR class) at 3 W, or 2 × 20 Ω 2 W in parallel on the same land — the Bourns CRM2512-FX-10R0ELF previously carried here is a 2 W part and must NOT be fitted" },
  "MLCC-1n-0402-C0G":  { status: "CLASS", spec: "1 nF 0402 C0G/NP0 ≥ 50 V — ADC sampling reservoir at every analogue card pin (E82 A2-09). C0G is the requirement, not a preference: an X7R reservoir has a voltage coefficient, and the thing it feeds is a 12-bit conversion" },
  "SIC-1700-1R-G15":   { status: "CLASS", spec: "SiC MOSFET 1700 V ≈ 1 Ω SPECIFIED AT V_GS 12–15 V — aux flyback switch (E82 M-04). Acceptance: R_DS(on) ≤ 1.1 Ω at V_GS 15 V / 25 °C, V_GS(th) ≤ 4.5 V, V_GS(max) ≥ 19 V, R_G,int stated", note: "E82 RETIRES the ORDERABLE C2M1000170D (C5713500): its 1 Ω is a V_GS = 20 V figure and the NCP1252 DRV pin clamps at 15 V typ / 18 V max (Rev 9), so the part was never fully enhanced and at the 9 V UVLO corner barely on. Candidate class: Infineon CoolSiC trench M1 IMBF170R1K0M1 (a +15 V/−5 V part) — confirm the package against the MR-23 placeholder land before the PO; never a guessed C-number" },
  "ISO-GBIAS-15-1W":   { status: "CLASS", spec: "reinforced isolated gate-bias module, 1 W, 15 V DC in (13.5–16.5 V), dual out **+15 V / −3…−4 V**, ≥ 5 kVrms 1-min, C_io ≤ 5 pF, SIP land per the QA01C footprint family. ACCEPTANCE: **+15 V ±5 % at 20–100 % load and ≤ +17.5 V at 5 % load** — the channels run at ≈ 17 % load, where an unregulated open-frame module rises +10…+18 %", note: "E82 (M-03) RETIRES the invented \"QA01C-15\": MORNSUN's catalogue has QA01C (+20/−4 V) and QA01C-18 (+18/−3 V) and NO −15 V-rail variant, so the BOM was naming a part that does not exist. Whether the frozen switch wants the +15 V or the +18 V variant follows ITS recommended V_GS (the SG2M023120LJ has no public datasheet; 1200 V / 23 mΩ Chinese dies are 18 V parts) — this row moves with the C-10 device freeze. Candidates to qualify: MORNSUN QA01C-18, RECOM RxxP2xxxD reinforced, Murata MGJ1 class" },
  "ISO-GBIAS-15-2W":   { status: "CLASS", spec: "the same class at **2 W** — the LLC channels that drive TWO paralleled dies at 40/50 kW (E81 F-C-23: P_gate + driver I_CC2 reaches 1.59 W at f_max 203 kHz, 159 % of a 1 W module, and the droop hits the NSI66x1A 9.8–11.8 V VCC2 UVLO). Same acceptance row, same land", note: "see ISO-GBIAS-15-1W — same E82 M-03 reasoning and the same candidate list in the 2 W grade" },
