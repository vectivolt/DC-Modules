...
export const LCSC = {
  ...
  "CER-3W-R33-AX":     { status: "CLASS", spec: "0.33 Ω ≥3 W PULSE-rated axial, series L ≤ 100 nH — the resistor of the DC-link entry RC damper; it takes the link ring energy, so a chip part is wrong (1.8 W simulated at the PSM corner)" },
  ...
  "R2512-22k-HV-AS":   { status: "CLASS", spec: "22 kΩ 2512 3 W anti-surge HV, Umax ≥ 250 V, 2-series per half — DC-link balance (E81 F-A-7: 47 k gave 4.4 mA against a hot leakage imbalance up to ~17 mA)" },
  ...
  "QA01C-15":          { status: "REVIEW", mpn: "QA01C-15", note: "MORNSUN **+15/−3 V** 1 W gate-bias module — E81 (F-C-15): the registered second source is −4/+15 V STATIC with a +19 V transient absolute maximum, which the drawn +18 V plus a real gate loop's overshoot crosses. Same price class as the -18 variant. REVIEW = confirm the exact order code and the RFQ bulk-decoupling per the MORNSUN app note; C_io ≤ 5 pF for the F-C-24 CM budget" },
  "QA02C-15":          { status: "REVIEW", mpn: "QA02C-15", note: "MORNSUN **+15/−3 V, 2 W** gate-bias module for the LLC channels that drive TWO dies (40/50 kW) — E81 (F-C-23): P_gate + driver I_CC2 reaches 1.59 W at f_max 203 kHz, 159 % of a 1 W module, and a droop hits the NSI66x1A 9.8–11.8 V VCC2 UVLO. REVIEW = order code + the same C_io line" },
