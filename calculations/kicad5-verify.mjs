...
// files and diff it against design intent.
//
...
for (const block of libText.split(/^DEF /m).slice(1)) {
  ...
  for (const line of block.split("\n")) {
    if (line.startsWith("X ")) {
      const t = line.split(/\s+/);              // X name num posx posy len orient ...
...
for (const f of readdirSync(SRC).filter((x) => x.endsWith(".json")).sort()) {
  ...
  // --- parse ---
  const comps = [];            // {ref, lib, x, y}
  const wires = [];            // [x1,y1,x2,y2]
  const labels = [];           // {net, x, y}
  const notes = [];            // frame segments
  ...
  const pinAt = new Map();     // "x,y" -> [ref.pin]
  ...
  const netAt = new Map();     // root -> net name
  for (const l of labels) {
    const k = key(l.x, l.y);
    if (!dsu.p.has(k)) { floating++; if (problems.length < 40) problems.push(`${page.page}: label ${l.net} at ${k} touches no wire`); continue; }
  ...
  for (let a = 0; a < boxes.length; a++) for (let b = a + 1; b < boxes.length; b++) {
    ...
    if (p1.x0 < p2.x1 && p2.x0 < p1.x1 && p1.y0 < p2.y1 && p2.y0 < p1.y1) {
      overlaps++; if (problems.length < 40) problems.push(`${page.page}: symbols overlap — ${p1.ref} × ${p2.ref}`);
    }
  }
  if (notes.length % 4 !== 0) { openFrames++; problems.push(`${page.page}: ${notes.length} frame segments (not a multiple of 4)`); }

  totOk += ok; totWrong += wrong; totAbsent += absent; sheets++;
  const verdict = wrong + absent ? `FAIL ${wrong} wrong · ${absent} unconnected` : "PASS";
  console.log(`${verdict.padEnd(34)} ${page.page.padEnd(22)} ${ok}/${ok + wrong + absent} pins · ${comps.length} symbols · ${labels.length} labels`);
}

console.log(`\n== ${sheets} sheets · ${totPins} connected pins · ${totLabels} labels ==`);
