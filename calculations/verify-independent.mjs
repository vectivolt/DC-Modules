// verify-independent.mjs — clean-room end-to-end verification of EVERY board, kept as a
...
// [SYNC] (section L): the ONE exception to this file's no-reuse rule, and deliberately so —
...
const SK = {   // full bridge: crN × 33 nF · external Lr (D2 rev F) · tank class (A rms) · 0.05 mm litz area · bank film/electrolytic counts
...
for (const [sku, s] of Object.entries(SK)) {
  ...
  s.Iout = s.Imax;
  s.Ip1max = (Math.PI / (2 * Math.SQRT2)) * (s.P / 500) / 0.98;   // reflected load current (n = 2) at the 500 V HIGH-mode floor, rms
...
// Clean-room tank: two series-primary cells, each an S1–P–S2 interleave — 1-D MMF energy L = µ0·N²·MLT/(4b)·(2g + (2hS + hP)/3)
...
XF["50kwa"] = XF["50kw"];
// The MMF breadth in the 1-D energy model is the CONDUCTOR BAND (28 mm), NOT the 41 mm window
// height: leakage ∝ 1/b, so substituting the window under-states it by 41/28 = 1.46×. With the
// D2 nominals above this lands on the committed tank Lr to three figures.
...
for (const [sku, s] of Object.entries(SK)) {
  ...
  ck("C", `${sku} pulse-resistor energies inside class`, eDis <= cls && ePre <= cls,
    `RDIS ${f(eDis, 0)} J · RPRE ${f(ePre, 0)} J per resistor vs ${cls} J (${sku === "50kw" ? "CER-50W-AX 50 W part" : "25 W family point"})`);
  ...
  ck("C", `${sku} output blocking diode class`, s.Iout / s.dout <= 0.70, `${s.Iout} A on the ${s.dout} A module = ${f(100 * s.Iout / s.dout, 0)} %`);
  // OC observability inside the rail
  // The comparator thresholds themselves (F.01 line / F.11 tank) must be representable with
  ...
  ck("C", `${sku} bank bleeder window (film bank)`, tauB <= 17 && 0.5 * cBank * 500 * 500 / 4 <= 65,
...
{ // X-bleed: three star X2 stages (a star C is C/3 line-to-line) + the Δ damper cap (its 10 Ω is ≪ the bleed star).
  // The star resistor is read from the NETLIST, never assumed: adding an X stage without re-sizing the star pushes the
  // terminals past the 5 s rule for permanently connected equipment, so both the 1 s and the 5 s rule are checked.
  ...
  ck("C", "X-cap bleed after the star X2 stages + damper", Math.abs(rStar - 33e3) < 1 && tauX <= 1.0 && t60 <= 5.0,
    `${f(rStar / 1e3, 0)} k per star element (netlist) · τ ${f(tauX, 2)} s for ${f(cX, 1)} µF line-to-line equivalent ≤ 1 s pluggable rule · 672 V pk → 60 V in ${f(t60, 2)} s ≤ 5 s permanently-connected rule (a 47 k star reads ${f(t60 * 47 / 33, 2)} s here and 5.3 / 6.4 s on the per-phase-star form)`);
}
...
function tjWorst(sku) {
  // No row of the envelope is excluded: the ceiling check and the closed-form reproduction run on EVERY row, the same way
  // stress-audit does. An exclusion here would hide exactly the two defect classes the grid is meant to catch (a mistimed
  // weak-leg dead time, and a per-leg loss charged to each die of the leg).
...
{ // 50 kW liquid: re-derive the grid's OWN worst LLC row closed-form (full bridge, par FETs per position).
  // The grid carries a real per-position turn-off term, never a flat placeholder, so this file re-derives that term
  // INDEPENDENTLY from the three declared inputs (tanks.mjs k_off for the fitted C_s / R_g,off pair, the row's own bus
  // and I_toff, and f_sw = fn × fr computed from the tank's own L and C) and checks BOTH the watts and the junction.
  ...
  // the R_DS(on) slope is the proxy datasheet's 0.0054 /K (21 → 38 mΩ, 25 → 175 °C), typed here independently of the grid
  let Tj = 80; for (let i = 0; i < 40; i++) Tj = plate + ((ipW / Math.SQRT2 / par) ** 2 * 0.023 * (1 + 0.0054 * (Tj - 25)) + woffIndep / par) * 0.65;   // clip mount onto the plate
...
for (const [sku, s] of Object.entries(SK)) {
  ...
  ck("E", `${sku} aux load inside the 110 W stage`, est <= 93,
    `${nBias} bias modules + coils ${f(coils, 1)} W + fans ${fansW} W (${s.fans} fitted) + logic ≈ ${f(est, 0)} W ≤ 93 (85% of 110)`);
}

// ---------- F. bank + link ripple (3-φ interleave, numeric) ----------
console.log("\n=== F. CAPACITOR RIPPLE (numeric interleave model) ===");
for (const [sku, s] of Object.entries(SK)) {
  // one bridge, no interleave — the full-wave rectified ripple √(π²/8 − 1)·Idc at the HIGH-mode floor lands on the rectifier film
...
for (const [sku, s] of Object.entries(SK)) {
  ...
  ck("G", `${sku} fans + tach terminators`, cnt(A, /^JFAN\d$/) === s.fans && cnt(A, /^RFDT\d$/) === (s.fans === 0 ? 4 : 0),
    `${s.fans} fans${s.fans === 0 ? " + 4 defined-low tach terminators (sealed; every tach way is defined)" : ""}`);
  ...
  ck("G", `${sku} DC-DC default-OFF coverage`, dcPD.every((n, i) => D.netOfPin.get(`RPDB${i}.pin1`) === n && D.netOfPin.get(`RPDB${i}.pin2`) === "DGND") && !D.byName.has(`RPDB${dcPD.length}`),
    `${dcPD.length}/6 lines pulled to DGND — no output-contactor or pre-insertion line among them`);
  // LLC + tank + banks
  ck("G", `${sku} LLC full bridge`, cnt(D, /^Q[12][HL]$/) === 4 && cnt(D, /^Q[12][HL]2$/) === (s.parL >= 2 ? 4 : 0) && cnt(D, /^Q[12][HL]3$/) === (s.parL === 3 ? 4 : 0) && !D.byName.has("Q3H"),
    `${4 * s.parL} FETs, 2 legs × ${s.parL} per position`);
  ...
  // The DC-DC entry film, with a 2.2 µF + 0.33 Ω series-RC across the same rails at the bridge. Too FEW films is the
  // hazard: four 1 µF parts put the bank on a 268–425 kHz anti-resonance with the stud loop, inside the 280–406 kHz
  // 2·fsw band, and the cans then carry 6.5–18 A rms against ≈ 3 A instead of 0.77 A/can, with the bus ringing 209 V
  // instead of 37 V. The count is PER SKU: on the 40 nH-stud deck 16 films hold the 30/40 kW cans at 58/68 % of the RFQ
  // ripple line, while the 50 kW pair needs 20 (16 reads 81 %). Independent re-declaration here; parts-db ENTRY_FILM and
  // boards.tsx are the other two carriers, tied to each other by current-coordination [SYNC].
  const N_ENTRY_FILM = { "30kw": 16, "40kw": 16, "50kw": 20, "50kwa": 20 }[sku];
  ck("G", `${sku} DC-link entry film + RC damper`,
  ...
  ck("G", `${sku} film-only banks`, cnt(D, /^CF[AB]\d+$/) === 2 * s.nF && !D.byName.has("LFA") && !D.byName.has("CEA0") && D.netOfPin.get("CFA0.pin1") === "BKAP" && D.netOfPin.get("CFB0.pin2") === "BKBN" && D.netOfPin.get("D1A1.pin2") === "BKAP",
    `per bank ${s.nF}× 2.2 µF film across BK·P–BK·N, JBS cathodes straight onto the bank`);
  ...
  ck("G", `${sku} output: blocking diode, no K_OUT / pre-insertion`, D.netOfPin.get("DOUT.pin1") === "BKAP" && D.netOfPin.get("DOUT.pin2") === "OUTP" && !D.byName.has("KOUT") && !D.byName.has("KPREA") && !D.byName.has("RPREA") && !D.byName.has("UEXCL2"),
    "BKAP → DOUT → OUTP · matrix KSER/KPARA/KPARB only");
  ck("G", `${sku} RATING strap`, Math.abs(D.val.get("RROLEB") - { "30kw": 0, "40kw": 1000, "50kw": 10000, "50kwa": 15000 }[sku]) < 1, `${D.val.get("RROLEB")} Ω`);
  // the filter the pre-compliance and stability gates model — Y trios on BOTH CM-choke nodes (two CM stages)
  // and the CX2-node Rd–Cd damper; then the PE leakage those Y caps imply. The CONVERTER-node trio is 10 nF,
  // the inter-choke trio 4.7 nF.
  ...
  ck("G", `${sku} two-stage CM ladder`, [1, 2, 3].every((p) => yOn(`AC${p}`, 10e-9) === 1 && yOn(`AC${p}M`, 4.7e-9) === 1 && A.netOfPin.get(`CMC1.B${p}`) === `AC${p}M` && A.netOfPin.get(`CMC2.A${p}`) === `AC${p}M`),
    "Y2 10 nF L-PE on the converter node AC1..3 (the LLC bridge is a CM source in its own right) and Y1 4.7 nF on AC1M..3M between CMC1 and CMC2");
  // the damper is per SKU — 50 kW (liquid and air) 4.7 µF / 4.7 Ω, 30/40 kW 2.2 µF / 6.8 Ω. Undamped to lightly
  // damped, the filter's LCL modes oscillate at 75.6 % (40 kW) / 194.7 % (50 kW) of the 15 µs control delay.
  ...
  ck("G", `${sku} CX2-node damper`, [1, 2, 3].every((p) => A.netOfPin.get(`CDMP${p}.pin1`) === `AC${p}` && A.netOfPin.get(`CDMP${p}.pin2`) === A.netOfPin.get(`RDMP${p}.pin1`)
    ...
    `${dmpC * 1e6} µF + ${dmpR} Ω in series, delta across AC1..3 (value per SKU)`);
  // the InfyPower filter — three star X2 stages (4.7 µF each, own floating star), no DM choke
  ...
  // This gate is LEFT HONEST: the 3.5 mA line is a charger-level PE-architecture constraint, not a number to widen when
  // a change crosses it. The converter-node trio is 10 nF for the LLC bridge's in-band CM fundamental (+3.1…+7.1 dB
  // instead of +0.2 dB), and the rack architecture is ONE PE CONDUCTOR PER MODULE — each module's PE pin bonds to the
  // rack PE bar, the backplane practice of every vendor module read — so the 3.5 mA line applies per module. The
  // alternatives if that ever changes: 6.8 nF (−3 dB of CM margin), or two modules per shared conductor.
  ck("G", `${sku} Y leakage to PE (one line open, one PE conductor per module)`, iPE <= 3.5e-3, `${f(cyPh * 1e9, 1)} nF/phase → ${f(iPE * 1e3, 2)} mA per module vs the 3.5 mA line on its own PE conductor (EVT T-14; three modules on ONE conductor would read ${f(3 * iPE * 1e3, 2)} mA — the rack rule is one conductor per module; the 6.8 nF alternative costs ≈ 3 dB of CM margin)`);
...
ck("H", "discharge windows match physics", /3000u/.test(fsmc) && /4000u/.test(fsmc) && /5000u/.test(fsmc),
  "3000/4000/5000 ms vs powered-path 1.98/2.37/3.16 s — the window covers the AC-PRESENT latch case; AC-removed is two-phase (active→321 V, then passive to 60 V — protection-thresholds carries the label time)");
...
// ---------- I. electrical proofs from the netlists: bias returns, rails, aux loop, exclusion, anode lock ----------
console.log("\n=== I. ELECTRICAL PROOFS — bias returns, rails, aux loop, exclusion, anode lock ===");
for (const [sku, s] of Object.entries(SK)) {
  const A = B[sku].ac, D = B[sku].dc;
  // bias COM ≡ driver GND2 ≡ FET Kelvin source, one node per channel (sample per stage)
  ...
  // every board sources V3P3 locally/through its slot, EN inside its abs max
  ...
  // aux loop closed with the right sign
  ...
  // hardware S/P exclusion truth-table wiring
  ck("I", `${sku} S/P hardware exclusion`, D.netOfPin.get("UEXCL.A1") === "CTL_KPARA" && D.netOfPin.get("UEXCL.B1") === "CTL_KPARB" && D.netOfPin.get("UEXCL.A2") === "CTL_KSER" && D.netOfPin.get("UEXCL.Y4") === "KSER_STG1",
    "KSER_STG1 = KSER AND NOT(KPARA OR KPARB) — series and parallel can never be commanded at once");
}
// payload anode name ↔ netlist anode net lock — the render-seating defect class, held at the data level
{
  ...
  for (const c of pay.chunks.flat()) {
    ...
    // netlist convention: port pin1 IS the anode. The payload's pin NUMBERING is inverted for
    // diodes while its "A" NAME is correct, and the sheets seat by that name, so this lock pins
    // name ↔ netlist and neither layer can drift away from the other.
...
// ---------- J. electrical proofs from the built netlists: reset chain, comparators, gate drive, discharge ----------
console.log("\n=== J. ELECTRICAL PROOFS — reset chain, comparators, gate drive, discharge ===");
// the watchdog verdict RESETS the brain (card) — one open-drain wire-OR node
{
  const C = B.card;
  const nrst = C.netOfPin.get("UCARD.pin14");
  // the WDO→NRST leg is a 0 Ω LINK (RWDOLCARD). Without it a blank chip resets
  ...
  ck("J", "card watchdog kick is terminated and probeable", C.netOfPin.get("RWDICARD.pin1") === C.netOfPin.get("USUPCARD.WDI") && C.netOfPin.get("RWDICARD.pin2") === "DGND" && C.netOfPin.get("TPWDICARD.P") === C.netOfPin.get("USUPCARD.WDI") && C.netOfPin.get("USUPCARD.VDD2") === C.netOfPin.get("USUPCARD.VDD"),
    "WDI 10 k pull-down (a FALLING-edge-triggered input must not float) + fixture test point · TPS3430 VDD2 bonded to VDD1, which the datasheet makes mandatory");
  ck("J", "card AND-gate bypass", C.netOfPin.get("CANDCARD.pin1") === C.netOfPin.get("UANDCARD.VCC") && C.netOfPin.get("CANDCARD.pin2") === "DGND",
    "100 n at the safety AND VCC");
  // the merge must be VISIBLE — the canonical net NAME on every one of those pins is NRST_CARD
  // itself. A net-net trace can be electrically right and still draw as two label groups, which
  // reads as "watchdog not connected" off the face.
  ck("J", "card WDO face-name = NRST_CARD", nrst === "NRST_CARD" && C.netOfPin.get("RWDOLCARD.pin2") === "NRST_CARD",
    "one NAME, every pin — the sheet shows the connection the netlist carries. The pull-up sits on the WDO side of the 0 Ω link, so the link's far pin is what carries the NRST_CARD face name");
  // Comparator-INSTANCE-aware current-sense allocation: no two phases may share one comparator.
  // AIN8 and TSNS0 sit the way they do because phase A's current sense is the ONE signal of 75
  // that does not carry to the second-source STM32G474VET7, where pin 17 (also PC2) lists
  // ADC12_IN8 and NO comparator.
  ...
  ck("J", "card CMP allocation (AIN8 ⇄ TSNS0 dual footprint)",
  ...
  ck("J", "card 8 MHz crystal on OSC_IN/OSC_OUT",
  ...
  // the F.03/F.13 "HW comp" rows have comparators behind them — SNS_VBUSP on PB13 (CMP4_IP, ref
  // DAC3_OUT0, HRTIMER fault ch 5) and SNS_VOUT on PA1 (CMP0_IP, ref DAC0_OUT0 internal-only
  // MODE0=011, fault ch 3). PE15/PA5 carry no usable comparator, so these senses cannot move there.
  ck("J", "card OVP CMP allocation", C.netOfPin.get("UCARD.pin52") === "ANA14" && C.netOfPin.get("UCARD.pin21") === "AIN3",
    "SNS_VBUSP→PB13/CMP4_IP · SNS_VOUT→PA1/CMP0_IP (GD32G553 DS Rev 1.01 Table 2-4 + UM Table 25-21 verified)");
  // The netlist and the DELIVERED KiCad face can disagree: the kicad5 chain has an upstream stage
  // (sheet-pages: dist → page tables) that a partial regen can skip, and kicad5-verify only proves
  // face ⇄ payload, so a stale payload verifies clean. This check reads the SHIP .sch/.lib FACE
  // itself and traces the §J pins to their label stubs, so payload staleness cannot pass.
  {
    ...
    ck("J", "card SHIP FACE pin ownership", Object.entries(WANT).every(([n, w]) => faceOf(n) === w),
      Object.entries(WANT).map(([n, w]) => `pin${n}=${faceOf(n)}${faceOf(n) === w ? "" : "≠" + w}`).join(" · "));
  }
}
for (const [sku] of Object.entries(SK)) {
  const A = B[sku].ac, D = B[sku].dc;
  // DESAT series R, blanking cap stays driver-side (PFC + LLC samples cover both cell uses)
  ...
  // bypass at every pin class that needs it
  ...
  // exclusion stage — the KSER coil input is gated
  ...
  // output-current sign — VINP rides KB (OUTN side) so delivering current reads POSITIVE
  ck("J", `${sku} shunt differential sign`, D.netOfPin.get("USHO.VINP") === D.netOfPin.get("RSHO.KB") && D.netOfPin.get("USHO.VINN") === D.netOfPin.get("RSHO.KA"),
    "return current OUTN→B→A: KB high of KA when delivering — positive reading = charging");
  // aux and HMI bypass + the cold-start reservoir at the pins
  ck("J", `${sku} aux + HMI bypass`, A.netOfPin.get("CVCCB.pin1") === A.netOfPin.get("UAUX.VCC") && A.netOfPin.get("CVCCB.pin2") === A.netOfPin.get("UAUX.GND") && A.netOfPin.get("CVCC.pin1") === A.netOfPin.get("UAUX.VCC") && D.netOfPin.get("CSR1.pin1") === D.netOfPin.get("USR1.VCC") && D.netOfPin.get("CSR1.pin2") === "DGND",
    "NCP1252 VCC: 100 n at the pin + 220 µF cold-start reservoir; 74HC595 decoupled");
  // Balance-string count: ONE network per MODULE on every SKU (cells.tsx `bal`, boards pass bal={k === 0}). The 40/50 kW
  // links carry TWO bank blocks in parallel, so fitting the pair per block would fit it twice and bury tens of watts of
  // standby inside the electrolytic bank. The passive discharge rides these same resistors, so the count, the VALUE and
  // the two end nets are all proven from the netlist rather than modelled.
  ...
  ck("J", `${sku} link balance population`, nSets === 1 && Math.abs(rBal - 47e3) < 1 && A.netOfPin.get("RBALT0A.pin1") === "DCP" && A.netOfPin.get("RBALB0B.pin2") === "DCN"
    ...
    `${nSets}× (2×${f(rBal / 1e3, 0)}k) per half drawn → 188k full-link on EVERY SKU → 321→60 V in ${f(tPas, 0)} s = ${f(tPas / 60, 1)} min; banks 1/2 carry cans only`);
  // PV bleeder drive at the guaranteed point — V15-fed LEDs behind the shared low-side
  ck("J", `${sku} PV bleeder drive network`, D.netOfPin.get("RPVLA.pin1") === "V15" && D.netOfPin.get("RPVLB.pin1") === "V15" && D.netOfPin.get("UPVA.CAT") === "PV_SINK" && D.netOfPin.get("UPVB.CAT") === "PV_SINK" && D.netOfPin.get("QPVD.C") === "PV_SINK" && D.netOfPin.get("QPVD.E") === "DGND" && D.netOfPin.get("RPVDP.pin1") === D.netOfPin.get("QPVD.B") && Math.abs(D.val.get("RPVBA") - 6.8e6) < 1e3 && Math.abs(D.val.get("RPVLA") - 1000) < 1,
    "1 k/2010 LED feed holds ≥10 mA to the 13.5 V rail floor via QPVD; 6.8 M gate bleed (25 °C-endpoint model, EVT gates the FET)");
  // symmetric parallel gate branches — every paralleled device behind its OWN 2.2 Ω
  const lpar = D.byName.has("Q1H2");
  // the VIENNA common-source pair carries a per-die 1 Ω de-Q on EVERY SKU: two 5–6 nF C_iss gates
  // tied directly together through package lead inductance are an undamped 50–200 MHz loop that no
  // simulation covers (the DPT has one device). No bare gate branch anywhere — every die sits
  // behind its own series resistor.
...
// ---------- K. switched models vs peer-reviewed closed forms and a measured reference ----------
console.log("\n=== K. Vienna vs Friedli/Kolar closed forms · LLC vs Wolfspeed CRD-30DD12N-K measurement ===");
...
// Anchor: the one-bridge tank current at the HIGH-mode floor against the full-wave first-harmonic closed form — load current
// P/(0.9·n·Vbank) in quadrature with the simulated magnetizing peak/√3
{
...
// ---------- L. [SYNC] — the drawn turn-off snubber vs tanks.mjs ----------
...
// leave the schematic behind — the same drift class mag-sync exists to stop for magnetics.
console.log("\n=== L. [SYNC] — drawn LLC turn-off snubber vs tanks.mjs `cs` ===");
{
  for (const [sku, s] of Object.entries(SK)) {
    ...
    ck("SYNC", `${sku} LLC turn-off resistor fitted at 0 Ω, land kept`,
