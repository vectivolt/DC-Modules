...
// ---- B: PFC choke
{
  let rippleFail = 0, satFail = 0, worstR = 0;
  const muPU = (H) => 1 / (1 + 2.13e-4 * Math.pow(H / 79.577, 1.637));
  for (let i = 0; i < N; i++) {
    const kAL = unif(0.88, 1.12);
    const N36 = 36, le = 0.201, Ae = 2.62e-4 * 3, AL = 4e-7 * Math.PI * 26 * 2.62e-4 / 0.201 * 3 * kAL;
    let dImax = 0;
    for (let k = 1; k < 40; k++) {
      const th = Math.PI * k / 40, iTh = 77.7 * Math.sin(th);
      const L = muPU(N36 * iTh / le) * AL * N36 * N36;
      dImax = Math.max(dImax, (269.4 * Math.sin(th) * (1 - 0.674 * Math.sin(th))) / (50e3 * L));
    }
    worstR = Math.max(worstR, dImax);
    if (dImax > 26) rippleFail++;
    if (muPU(N36 * 88 / le) * kAL < 0.36) satFail++;
  }
  report.push(["B PFC choke ±12%", `worst ΔIpp ${f(worstR, 1)} A (design 21.4, OC threshold headroom to 105 A pk)`, `ripple>26 A: ${100 * rippleFail / N}%`, `soft-sat floor fail: ${100 * satFail / N}%`, rippleFail === 0 && satFail === 0 ? "PASS" : "CHECK"]);
}
// ---- C: Vout accuracy
{
  let pre = [], post = [];
