...
| ADC health | every tick | — | — | the boot reference within ± 5 %, the live reference tracker within ± 2 % of it (integrating each clean-cycle estimate once and holding through a timeout or a disturbed cycle), and the AVMID bias at half the rail (1.65 V ± 50 mV on the nominal scale, uncorrected — it is ratiometric), each for 100 ms → F.29 |
