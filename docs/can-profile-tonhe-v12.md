...
|---|---|---|---|---|
| M_C_1 | 0x000100 | 6 | 500 ms and on change (spaced ≥ 50 ms) | state u8 · output voltage u16 0.1 V · output current u16 0.01 A · fault word u16 · PFC byte |
...
| M_C_4 | 0x009100 | 7 | 500 ms and on change | state bits u16 · fault / warning bits u16 · 4 × 0x00 |

The periodic frames are phased by address (37 ms × address, modulo 500 ms), so a rack of modules does not transmit in one burst.
...
| fault | 8 | input power limit | E1 input derate |
...
| Output-voltage mode | not in V1.2 | automatic: LOW ≤ 500 V, HIGH above (E67) |
| Address | §9.2.5–9.2.6 | automatic = the HMI address · manual = the CAN-assigned address · stored · changed only with the output off |
...
| TH-AMB-5 | "automatic" address assignment has no mechanism | automatic = the module's local HMI address; no valid address → the module stays silent | the externally observable equivalent | — |
...
| cadence | 20 ± 2 frames of each periodic PGN in 10 s; a new fault reported within 50 ms |
