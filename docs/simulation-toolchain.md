...
>
> **What runs where** — the SPICE decks are run by hand and their result CSVs are committed;
> [`current-coordination.mjs`](../calculations/system/current-coordination.mjs) and
> [`magnetics-envelope.mjs`](../calculations/magnetics/magnetics-envelope.mjs) then read them inside `run-all.sh`.
> No JS engine calls ngspice, and the PyOpenMagnetics cross-check never gates the battery.

## At a glance

| | |
|---|---|
| **Engine** | **ngspice-46** (KLU, `method=gear`) on macOS arm64 · Node 20 for the JS engines · PyOpenMagnetics 1.4.0 by hand |
| **Levels** | L1 closed form → L2 JS engines → L3 averaged system grid → L4 switched SPICE → **L5 hardware (EVT T-00…T-79)** |
...
| **Rule** | a simulation must be able to **fail** — every deck carries a physicality guard, a power-solve and a model fingerprint, and every guard in §7 is kept honest by an injection that must make it fail |
...
```mermaid
flowchart TB
  L5["L5 · hardware (EVT T-00…T-79)<br/>DPT · SC · calorimetric η · chamber EMI · thermal"]
  ...
  L3["L3 · system / statistics — JS + C<br/>envelope grid · Monte-Carlo 10k per batch · FSM scenarios · C host suites<br/>current-coordination: F.11 race · bypass-closure inrush"]
...
| **C host-sim** | L3 production logic: FSM scenarios, CAN codec with a 1 M-frame fuzz per profile, the group share law on three nodes | `sh firmware/run_tests.sh` → **seven binaries under Address / UB sanitizers**; the script prints the check count | runs the shipped C99 core itself, not a model of it | no HAL, no real peripheral timing |
...
| **PLECS** (commercial) | optional closed-loop + thermal co-simulation of the shipped controllers | piecewise-linear switches + datasheet thermal tables | industry standard for converter control and thermal studies | licence; not needed while the C host suites carry the control logic |
...
| `ZVS` | switches that turned on with the leg already at the far rail, out of 64 edges | 64/64 on every corner except the registered weak-leg (leg A) exceptions in phase shift, which read 32/64 |
| `t_dead_used_ns` · `Vres_A_V` · `Vres_B_V` | the dead time the deck programmed on that corner, and the volts each leg still had on it when the incoming device turned on | leg B always 0 V. Leg A is 0 V outside phase shift; inside it the residual is the number `current-coordination [ZVS]` registers and `envelope-grid` charges as a hard turn-on |
...
the true peak, so it is larger than the nominal-L ripple. Dips and a 20° phase jump add a few amperes only because the
current reference is clamped at 1.05 × during the event. The 150 kHz band content must sit below the LISN basis. A THD
rise at 475 / 500 VAC on a 650 V bus is a bus-policy defect, not an artefact — the line-tracking bus floor removes it.
...
| `aux-flyback` per SKU | V24 inside the consumer window (settle ≥ 22.5 V, dip > 21 V, ≤ 26.4 V), V15 above its 12.5 V floor, and the short-circuit rows | every PASS row, residuals recorded |
...
| **Verdict** | the worst corner for each criterion, with Bsat(T) of N95 falling from 525 mT at 25 °C to 410 mT at 100 °C | `[CONTROL]` must reject the control-group section transformer put in the one-bridge cell duty | end-of-life tolerance corners count for flux and saturation only, never as thermal states |
...
| `[CONTROL]` | the gate still rejects the control-group section transformer when it is put in the one-bridge cell duty | "PASSES — the gate is blind": a constant or boundary change has removed the gate's ability to fail. Treat every `ok` as unproven until the control is rejected again |

**Field guide — a `[D3]` line** (40 kW, from the current evidence):

```text
ok    [D3] 40kw 3×E70 2 cells 4:4∥4 (2 foils) · web2 · R core→wall 0.52 · winding→core 0.58 K/W — core corner ENV500-55 87.4 kHz B̂ 158 mT Fe 47.2 W · copper corner SER250-full-bus764 203 kHz Cu 46 W · hot-spot 102 °C @55 °C (SER250-full-bus764; core 89 °C / winding 102 °C) / 103 °C @75 °C derated (ENV500-55) · +25 % Rth 108 °C · runaway margin 116 K · B̂ 39 % of hot Bsat
```
...
| `core corner … B̂ 158 mT Fe 47.2 W` | the corner with the most core loss — for D3 always a high-bank, below-resonance envelope corner | — | more turns or another set (B̂ ∝ 1/(N·Ae)); a power derate changes nothing |
| `copper corner … Cu 46 W` | the corner with the most copper loss — the SER 250 V bank corner on the bus floor, where the tank current peaks | — | strand count, foil thickness, mean turn |
...
| `runaway margin 116 K` | distance from the hottest core equilibrium to the temperature where the core loop gain reaches 1 (at +25 % Rth) | ≥ 25 K | lower B̂, or lower the core→wall resistance |
...
| temp-critique `[BSAT] D2 fault flux` | L(max) × the window-comparator kill peak against 60 % of Bsat(130 °C) = 217 mT: **154–158 mT** | a faster kill or more D2 N·Ae |
| current-coordination `[F.11] window-comparator kill + observability` | when \|Ip\| crosses F.11 on `llc-short.csv`; the kill peak 1 µs later ×1.2 must stay under the CT ceiling (**210.3 A × 1.2 against a 344.7 A limit** at 30 kW); the +3 µs monitor peak ×1.05 inside the rail; both window thresholds inside the ADC span | re-burden or re-threshold, then re-run `ct-frontend`. Never return to fixed-time sampling |
...
| Row | What it proves | Result | How to read it |
|---|---|---|---|
...
| Closed-loop LLC load steps and Vienna dip recovery on the real HAL | PLECS co-simulation | EVT |
...
| **control group** — a section transformer known to be wrong for this duty must be rejected on every SKU | `magnetics-envelope.mjs` → `[CONTROL]` | a model or constant change that leaves the gate unable to fail |
