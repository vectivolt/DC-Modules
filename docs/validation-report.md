...
| **The enable write after a trip** | each control interrupt decided its outputs from `trip_n == trip_ack` and then wrote CHOUTEN; a fault landing between the two had already killed every output in hardware and the write re-armed them — for one control period, or for good, since the timer resumes a pulse fault the moment the source is gone; the tick's re-arm also cleared fault flags the fault interrupt had not yet consumed | enable authority is one place: the timer apply functions only return the outputs that should be on, and `outputs_commit` writes CHOUTEN on an output's OFF → ON transition alone — a stage start, a burst restart, the supervisor's re-command — inside a ≈ 100 ns critical section that checks the supervisor's acknowledgement and the timer's own fault flags (set in the kill's clock domain) before the write and the flags again after it. A completed fault pulse is never followed by a stale enable; a fault inside the write's own bus cycle is undone in the same section; the tick-side re-arm path is gone and only the fault interrupt clears flags | `review-checks` `TRIP-GUARD-AFTER-ENABLE` · T-44 fault-at-every-boundary injection |
...
| **Control path reaching into flash** | the two control interrupts are meant to run from TCM so a flash append never stalls them (slot B shares its bank with the journals); the placement rules listed functions by name, and the LLC's ZVS table, the PFC timer table, the regulator's reset, `memset` and two new helpers — one of them out-lined with a veneer by the compiler — were in flash | the control path's objects go to TCM with their tables and `memset`; the helpers are forced inline; the build walks the linked image from the three interrupts and fails on any branch, indirect call or literal into flash | `build.sh` TCM audit (fails when a callee is moved back) · `review-checks` `OUTPUTS-COMMIT` · T-44 |
| **Reference drift after boot** | VREFINT is read once at boot (its 17.1 µs aperture cannot fit the 10 µs frame), so every reading and every comparator code carried the rail's later movement — 885 V for the 860 V trip at +1 %, past the 880 V bench line, and the buck's datasheet bounds only the population, not one unit's drift | the reference is tracked live from the common DC of the three AC channels, which the line cannot produce (meas.h): an integrator bounded to ±2 %, off on an uncalibrated card, F.29 beyond the bound; the trip follows the rail instead of drifting 25 V per percent | `app_test`: VREFP moved ±1 % after boot without a reboot keeps the F.03 crossing at 860 ± 3 V, +5 % is F.29 (884.7 V without the tracker) · `hal_test` common / residual split · `review-checks` `VREF-LIVE-TRACKER` · T-26 |
...
| the reference tracker's floor | the live reference reads VREFP against the three AC isolators' output common mode; a common drift of those three with temperature is indistinguishable from the rail moving, and no vendor number for it is on file | a ±0.2 % class (bandgap-derived outputs, tens of µV/°C) → F.03 in 855–865 V, both threshold inequalities closing with 13 V to spare against the 880 V bench line; T-26 moves VREFP after boot without a reset and measures the crossings against an external instrument, and a floor above ±0.5 % re-opens the trip budget |
