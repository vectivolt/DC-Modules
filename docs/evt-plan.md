...
| T-26 | PFC fast-trip timing, BOTH current polarities | measured threshold-crossing → gate-off ≤ the registered budget on every phase; CT polarity ↔ comparator-trip polarity confirmed per phase; DESAT covers the forward direction, the comparator the reverse; with VREFP moved ±1 % AFTER boot (bench supply on the card's 3.3 V, no reset, no re-calibration; ±2 % is the tracker's F.29 bound, so a +5 % move must latch F.29 instead of tracking — `app_test`) the tracker follows within 1 s and the measured crossings of every comparator stay within ±0.3 % of the programmed volts against an external instrument — the codes are the inverse of the corrected measurement and the reference is live; the residual is the isolators' common-mode floor and re-opens the trip budget above ±0.5 % |
...
| Test | Procedure | Pass criterion |
|---|---|---|
| T-44 | **Firmware timing on the target** — DWT per ISR for 24 h at full telemetry and under a CAN flood; GPIO markers on the reference commit and on GATE_EN; analyzer timestamps | PFC ISR ≤ 3 µs · LLC ISR ≤ 30 µs · 1 ms tick ≤ 400 µs · CPU ≤ 60 % steady, ≤ 75 % under flood · control frame → reference ≤ 5 ms (99.9 %) · FAULT_BITS ≤ 20 ms after an injected latch (N-11, F-04) · a watchdog and a software reset at every discharge transition resume the bounded dump, never precharge · a fault pulse injected at every boundary between the enable decision and the CHOUTEN write never re-arms an output · **before the first energisation**: with EN_PFC held low, each Vienna phase pin shows one pulse centred on the roll-over whose width follows CMP0 and no pulse at CMP0 = CAR (the centre-aligned single-crossbar rule), and the first cycle after a burst restart runs on the compare the restart wrote, not the previous one · no fault flag is set between boot and the first tick with the comparator references at full scale, and a fault line held asserted raises one interrupt, not a storm |
...
| T-49 | **Fault and recovery cycling** — 1 000 cycles each of grid sag, phase loss, over-temperature (heater on the NTC), communication loss, relay feedback open, watchdog reset and aux brownout; the IEC 61000-4-34 class 3 dip and interruption profile (0 % for one cycle, 40 % for 10, 70 % for 25, 80 % for 250 cycles, and a 250-cycle interruption) applied with a +1 % VREFP move in progress at the cut | every cycle ends in the documented state and recovers by the documented rule · no lockout from grid rows · every dip and the interruption end as AUTO_EXT F.07 / F.08 or a ride-through, never F.29, with the reference tracker held through the cut and re-converged before the LLC restarts · no spurious gate pulse (D-03, D-05) |
| T-52 | **Boot and update on the target** — 100 field updates over CAN at each bit rate incl. power cuts at every stage (block, FINISH, trial boot 1–3, confirm), a corrupted image, a foreign signature, a downgrade below the baseline, and the reset-streak path into safe mode | the confirmed image always runs · no cut leaves an unbootable module · every refusal matches `boot_test`'s codes · trial confirm at 60 s healthy standby exactly, and never while a stage runs · a watchdog reset logs F.32 and a commanded reboot does not (the reset cause crosses the handoff record) · a service session the tool abandons returns to the application after 60 s |
...
| T-63 | **F.01 on both CT orientations** — line-CT fitted both ways in turn; current injection to the 120 / 155 / 195 A pk line in both half-cycles. The comparator references are **positive-only** — a sub-AVMID threshold on a non-inverting comparator into an active-high fault input would be a standing fault — so the negative polarity is the 100 kHz software magnitude trip (T-70) | the hardware trip fires within the row's class on the POSITIVE half-cycle for both orientations, the software trip answers the negative one inside the interrupt that samples it (≤ 19 µs after the crossing, before the CT's lag), and Σi = 0 gives the other two comparators a hardware answer at 2 × I_trip; attribution F.01-A/B/C correct |
...
| T-70 | **Negative-polarity F.01** — inject a negative line-current fault with the positive-only comparator references fitted | the **software magnitude trip stops the stage inside the interrupt that samples it** — ≤ 19 µs after the crossing (one sample period plus the interrupt's execution), before the CT's lag — and the outputs stay off until the supervisor has acknowledged the trip and commands the stage again |
