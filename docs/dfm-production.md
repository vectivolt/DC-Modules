...
> counterpart of the design pages. Every window below is the per-SKU value, and every end-of-line limit
...
| **Unique per SKU** | SiC die class · tank capacitor count · magnetics · film-bank count · fuse, relay and DOUT current class · shunt · busbar lengths · CT burden values |
| **End-of-line test** | 9 steps at 100 %, plus a sampling plan |
| **Coating** | acrylic conformal coating on both boards and the card |
| **First-article holds** | magnetics per lot · coating mask set · choke centre-bolt torque · clip force · the bonded-soak accept band |
...
flowchart LR
  ...
  T1 --> A5["5 · Conformal coat<br/>acrylic"]
...
| **3 · Magnetics fit** | D1 chokes on an M6 centre bolt through a GF-PPS clamp cap, gap pad on the bonded face, 2-point banding · D2 and both D3 cells with **both yoke faces gap-padded** to the upper and lower extrusion webs (coldplates on the 50 kW liquid), clamp bars, end turns potted · each D3 cell's measured leakage label recorded against the module serial — cells are not binned, EOL confirms fr · cutout thermostats series-wired into the T_XFMR loop | pad compression witness on every D2 / D3 face (feeler gauge against the pad's compressed thickness) |
| **4 · Board test** | Boards A (AC-DC) and B (DC-DC) tested separately at low voltage with a **test control card** in the harness: aux rails · card program and boot through its JSWD header (BOOT0 strapped low; the fixture drives `TPWDI` or connects under reset, because a blank chip is otherwise reset by the TPS3430 every 23 ms) · gate pulses into a dummy RC load (PWM-off isolation) · relay click **and mirror-contact readback** · HMI and CAN on board B | 100 % |
| **5 · Conformal coat** | Acrylic coating, both boards and the card — after board test so a fault is reworked before coating | mask set agreed at first article |
| **6 · Clip mount** | every TO-247 die to its extrusion or coldplate through a 0.635 mm Al2O3 insulator with thermal grease, held by a spring clip — the 0.8 K/W (air) / 0.65 K/W (liquid) junction-to-base basis every Tj gate uses | clip force within the clip vendor's band · grease coverage witness 1 / 50 · T-38 verifies the Rth at EVT |
...
| **9 · End-of-line** | The nine tests below → serialise, HMI address 00, ship configuration written over CAN IDENT | 100 % |

## 3. Torque schedule

| Joint | Hardware | Torque | Check |
|---|---|---:|---|
| TO-247 spring clip | clip screw per the clip drawing | per the clip vendor | clip force by load cell, first article and 1 / shift |
...
> [!NOTE]
> D1 rev B has no lug: it terminates in tinned flying leads soldered into ⌀ 6.0 mm plated holes, because a ring lug
> cannot land in a hole (D1 terminations on each module magnetics page).
...
| 4 | **Gate drivers, PWM disabled** | bias rails per channel · DESAT loop-back pulse · **blank-to-FLT: LLC 0.43–0.65 µs (18 pF), Vienna 0.80–1.38 µs (47 pF)** — a misfitted 100 pF reads ≥ 1.48 µs and fails the unit | the blank that sets the short-circuit response against SCWT |
| 5 | **Precharge and discharge** | precharge **145–250 / 175–300 / 235–400 ms** at 30 / 40 / 50 kW (deck t95 193 / 231 / 310 ms) · bus < 60 V within **3 / 4 / 5 s** (deck 1.99 / 2.39 / 3.19 s) · both banks < 60 V within the F.21b window **10.3 / 15.5 / 20.7 s** | F.20 · F.21 · F.21b windows |
| 6 | **Gain-capability calibration** | per-unit peak bank voltage at bus 830 V, low current → stored as `bank_max` · two-point V / I calibration in both directions · post-cal ± 0.18 % / ± 0.2 % (`monte-carlo.csv`) | the measurement chain |
...
| 8 | **Fault-injection subset** | OVP comparator (bus pump) · CAN timeout ramp-off · HMI buttons and segments · **F.01 / F.11 comparator landing by CT test-current injection at 120 / 155 / 195 A line and 140 / 180 / 220 A resonant (± 5 %, both polarities)** | the RATING strap selected the right DAC class and the right burden is fitted |
| 9 | **Bonded thermal soak** | 20 min at the 400 VAC rated point · T_XFMR NTC rise above inlet recorded and compared with the lot median — a unit more than the first-article band above the median is torn down; the band is set at first article by building one unit with a D3 face deliberately unbonded | a lost D2 / D3 bond, which `magnetics-envelope` shows is not survivable on D3 on every SKU and on D2 of the 50 kW pair |
...
> [!IMPORTANT]
> **Electrolytic vent rule** — keep ≥ 5 mm free space above every snap-in can's vent face; no conformal
...
> rule costs nothing and caps the outcome. The output banks are film-only and have no cans at all.

| Item | Specification |
|---|---|
| Coating | acrylic conformal coating, both boards and the card |
| Why | dust and condensation are the leading field failure for fan-cooled DC charging modules |
| Environment it serves | −30…+55 °C full power, derate to +75 °C · humidity 5–95 % non-condensing · vibration 2 g, 10–500 Hz (assumption A11) |
| Mask | the vent faces above; the rest of the mask set — TO-247 thermal faces, connector mating areas, bolted-joint contact faces, test pads — is agreed with the coater at first article |
| Cost | mechanical line ₹320–360 per module |
| Liquid 50 kW | the sealed, fanless enclosure covers the IP65 role — no potting |

## 6. What stays common across the family

| Common to every SKU | Unique per SKU |
|---|---|
| one SiC family (dies sized per SKU), one gate-driver part, one bias architecture | board outlines |
...
| one control card — its RATING strap tells it which SKU it runs | CT burden values |


## 7. Production programming and purchasing acceptance rows
...
> half of the safety function. It is also unnecessary: **`TPWDI` exists for this**. The fixture pogo-pins `TPWDI`
...
|---|---|---|
| Fans | electrical class **≤ 0.6 A at 24 V (≤ 14 W)** at 100 % PWM, start surge ≤ 1.5 A for ≤ 0.3 s | the 110 W aux is sized on it (i24s 1.6–2.4 A). The 160 m³/h class draws 7–14 W; a 25 W fan overloads the four-fan SKU into the NCP1252's latched over-current |
...
| Link damper resistor `RFDMP` | **`RTF-50W-R33-TO247`** — 0.33 Ω, ≥ 50 W thick-film non-inductive TO-247, clipped to the DC-DC heatsink, ₹95 @1k | it dissipates **7 / 16 / 36 W** per SKU whenever the bridge runs at f_max (2·f_sw = 406 kHz on the entry-film / stud resonance); the link deck reads 10.4 A rms = 35.6 W at 50 kW SER250 / 764 V. A 0.33 / 0.68 / 1.0 / 2.2 Ω sweep dissipates the same ≈ 40 W and lets the bus ring 127 → 185–206 V, so the value stays |
| Damper capacitor `CFDMP` | **I_rms ≥ 12 A at 400 kHz / 85 °C** | it carries the damper current at the same resonance |
| DC-link can `CD[TB]` | **ripple ≥ 5.2 A rms at 100 kHz / 105 °C, ESR ≤ 80 mΩ at 100 kHz** | the line must carry the Vienna's own 50 kHz capacitor current as well as the bridge's: **5.7 / 6.5 / 5.9 A per can at 400 VAC / 800 V**. At 340 VAC the duty is **6.7 / 7.6 / 7.0 A = 99–112 % of the allowance** — a declared exceedance with its levers on the purchasing line (forced air first, then a taller can, then + 1 can per half) |
