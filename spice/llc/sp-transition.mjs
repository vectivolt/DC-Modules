...
//
// The banks are FILM-ONLY: 9 / 12 / 14 × 2.2 µF per bank (current-coordination [SYNC] asserts that count against
// boards.tsx), i.e. 19.8 / 26.4 / 30.8 µF. The closure peak is ΔV/√(L/C), so what matters is the peak at the PAR permit
// |ΔV| ≤ 25 V, not at a ΔV ≤ 20 V sweep edge, and with this little capacitance the surge impedance — hence the LOOP
// INDUCTANCE — sets the answer outright. The deck therefore sweeps the loop inductance and reports the MINIMUM loop
// inductance the 25 V permit needs against the relay make line, which is a verifiable layout requirement.
// Weld detection is trivially observable here: dV/dt = I/C is 1.0 MV/s for 20 A into the film bank.
...
// film-only output bank per bank — the counts current-coordination [SYNC] asserts against boards.tsx
...
export const FW42_PERMIT_V = 25;         // fsm.c PAR permit: |ΔV| ≤ 25 V
export const LOOPS_NH = [50, 150, 300];  // 50 nH is a bare busbar estimate; a 9–14-part film bank spans ~240 mm of board
...
if (fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  for (const sku of skus) {
    ...
    const rows = [HDR];
    console.log(`\n=== ${sku} S/P closure: ${BANK_FILM[sku]} × 2.2 µF film per bank = ${f(C * 1e6, 1)} µF ===`);
...
.end`;
    ...
    console.log(`    (healthy banks diverge past a 1.5 V threshold in ${f((1.5 / dvdt) * 1e6, 1)} µs, so the detection window can be the FSM tick)`);
    writeFileSync(join(RES, "sp-transition.csv"),
      `# parallel-closure mismatch study; ${sku}: ${BANK_FILM[sku]} × 2.2 µF FILM-ONLY bank = ${f(C * 1e6, 1)} µF/bank\n` +
