...
## Schematic sheets (rev D.2, E34)

Release sheets with titled section frames live at `boards/<sku>/out/<side>-schematic.svg` —
generated by `node calculations/schematic-export.mjs` after `tsci build`. The layout is a
verified artifact: `node calculations/schematic-check.mjs` must report **0 symbol overlaps**;
cross-section connectivity is net-labels-only (`schSectionName` + `schMaxTraceDistance={0}`),
wires are reserved for local in-section connections.
