...
#   host_sim    — the supervisory core (fsm.c + group.c): the 26 §36 scenarios, the row checks, every-tick invariants
#   ctl_test    — the reference shaper and regulator kernel (ctl.c)
#   proto_test  — TonHe V1.2 and VMP 2.0 conformance and fuzz, and one core driven through both profiles
#   hal_test    — the portable real-time HAL on cycle-by-cycle Vienna and LLC plants, measurement, power-cut-safe NVM
#   app_test    — the module application end to end: interrupts, the 1 ms sequence, TonHe V1.2, faults, NVM, CAN
#   boot_test   — SHA-256, ECDSA-P256, the signed image, the boot decision and the update protocol on RAM flash
#                 the units driven directly, with no plant: the adaptive LLC dead time and demand map, the fsm rows,
#   rules_test    — the per-rating identity and fault channels, the TonHe/VMP profile rules, the bootloader's watchdog
#                 cadence and the junction observer
set -e
cd "$(dirname "$0")"
...
$CC -I. hal/app.c $HAL $CORE $PROTO boot/image.c boot/sha256.c boot/p256.c test/rules_test.c -o /tmp/pmp_rules_test -lm
/tmp/pmp_host_sim
/tmp/pmp_ctl_test
/tmp/pmp_proto_test
/tmp/pmp_hal_test
