...
/* E82 (M-20): all three journals live in bank 1. The flash has read-while-write ACROSS banks only (UM §2.3.4) — a write
 * into the bank a core is fetching from stalls CBUS, and with the factory image in slot A (bank 0) every configuration,
 * calibration, counter and event append used to stall interrupt entry for up to 20 ms. Bank 1 holds slot B, which is
 * never written while the application it contains is running, so nothing in the running path shares a bank with them. */
