/* boot_main.c — the bootloader image (32 KB at FM_BOOT). Kicks the TPS3430 from power-up (fixed window: first kick
...
/* nvmport.c's hook, called before every page erase and every programmed block. The bootloader
...
int main(void) {
  system_init();
  board_gpio_init();
  wdi_pulse();                                    /* the FIRST edge after tRST has no early boundary (TPS3430 §7.3: "the first
  ...
  /* EPRSTF (26) belongs in this mask exactly as it does in hal/app.c — the TPS3430's WDO is wire-ORed onto NRST, so
     the DOMINANT reset cause is an external pin reset, not FWDGT. Without it the streak never reaches BOOT_STREAK and
     safe mode is unreachable for every watchdog reset, which is the only kind this board produces. The module has no
     reset button (NRST goes to the supervisor and the SWD header alone), so nothing else sets it. */
  ...
  if (p.mode == BOOT_RUN) {
    /* hand the application a FRESH window — wait out the 5 ms spacing, service, jump */
  ...
  svc_t s;
  /* The service identity is a CRC over the WHOLE 96-bit device ID. RD(UID_BASE) alone is the wafer/lot word and is not
     unique between lots — two colliding modules would both answer SELECT, both from source 0xFE, and their identical
     identifier frames would destroy each other on the bus. The fixture reads the same CRC and prints it. */
  ...
  /* Join LISTEN-ONLY and stay there until a frame has been received at this rate. The handoff's bit rate is a guess on
     a cold boot, and an error-active node at the wrong rate answers every frame it mis-reads with an active error
     frame — a module power-cycled beside a working charger would destroy real traffic for the ~16 frames it takes to
     reach bus-off. Listening costs nothing: the service protocol always starts with a frame FROM the tool. */
  int listening = 1;
  can_init(bitrate, 1);
  pmp_txq_t tx;
  ...
  for (;;) {
    ...
    now_ms++;
    boot_poll();
    /* Drain until the mailboxes are empty. can_rx returns at most the 8 hardware mailboxes; one pass of 4 is below
       the ~3.6 frames/ms a 500 kbit/s DATA stream delivers, so a 1 KB block (147 frames, no per-frame flow control)
       overruns, the sequence check fails and the transfer restarts for ever. */
    uint8_t n;
    int seen = 0;
    do {
      pmp_frame_t f[8];
