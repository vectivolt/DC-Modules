/* can.c — the CAN1 classic 2.0B driver on the mailbox controller (facts-can.md).
...
uint8_t can_bitrate_bad;                       /* can_init was asked for a rate it has no table for */
uint16_t can_rx_congested;                     /* drains that found every mailbox occupied — frames may have been lost */
...
void can_init(uint32_t bitrate, int listen_only) {
  ...
  /* 250 k is an explicit branch and anything else falls back to it AND reports — a silent `else` would run the wrong
     bit timing for any caller that violated cfg_sanitize's {125,250,500} invariant. */
...
uint8_t can_rx(pmp_frame_t *dst, uint8_t max) {
  ...
  /* The controller reports no queue-overrun flag, but a pass that finds all eight mailboxes full is the
...
uint8_t can_state(void) {   /* 0 active · 1 passive · 2 bus-off (ERRSI 5:4; BOF sticky until restarted) */
  if (can_bitrate_bad) return 1u;   /* an unrecognised bit rate ran the 250 k table — report it as a CAN warning */
...
void can_restart(void) {   /* manual bus-off recovery: after BORF, ABORDIS 0 → resync on 11 recessive bits → ABORDIS 1 */
  ...
  if (e & BIT(19)) {
    ...
    /* No wait for SYN. Recovery needs 128 × 11 recessive bits — 11.3 ms at 125 k — so a spin here could only time out
       inside the 1 ms tick, and the UM is explicit that raising ABORDIS again only disables the NEXT recovery, never
       the one in progress. */
    CAN_CTL1(B) |= BIT(6);
  }
}
