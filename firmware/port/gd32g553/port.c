 ...
 * The 100 kHz and 10 kHz paths live in TCM (.ramfunc), and E82 (M-20) put the vector table there too, so flash appends
 * never stall them OR the exception entry that reaches them (§3.5, review HR-26). */
#include "port.h"
...
/* E82 (C-03): the internal reference, read once at boot with a legal 28.5 µs aperture (adc.c). It is no longer in the
   100 kHz sequence, so the tick reports this constant — same units (counts) the hal already expects. */
...
/* ---------------- 100 kHz: sample set from the previous roll-over trigger, control law, duties for the next roll-over.
   E82 (M-21): raised by the END of the longest ADC sequence (DMA0 channel 2 half/full transfer, adc.c CTL_DMACH), not by
   ST3 CMP3 — ≈ 7.2 µs before the roll-over that loads the compare shadows instead of 5.0, against a 4.6–6.1 µs ISR.
...
/* E82 (LV-2 / E-01): permission is a TOKEN COUNT the main loop must keep paying for, not a level. With the E81 sticky flag a
   hung main loop (a flash busy-wait that never ends, a corrupted app_t, any while() bug) left kick_arm = 1 for ever: SysTick
   kept servicing the TPS3430 AND the FWDGT while the 100 kHz / 10 kHz control interrupts went on delivering power on frozen
   references with every supervisory protection row dead. Each kick now consumes a token; tick() tops the purse up to
   PORT_KICK_TOKENS (30 ms of grace — one collapsed backlog), a flash erase buys its own bounded allowance through
   port_kick_grant(), and a dead loop is reset ≤ 30 ms + one TPS3430 window later. */
...
/* E82 (C-01b): nvmport.c calls this immediately before every flash operation and after every page erase. It buys the loop
...
/* E82 (lead item): `n` is how many SysTicks this call stands for, not a flag. The E81 form advanced `ms` by ONE per
   collapsed backlog, so every interval measured here stretched by exactly the time the loop had lost — the tach window
   below became 100 ms of TICKS instead of 100 ms of seconds and reported the fans slow, and the 1 Hz paint scan drifted.
   app_tick's own now_ms still advances by one per call; the one-line hal change that fixes it is in NOTES (not our file). */
static void tick(uint32_t n) {
  ...
  /* E82: a CROSSING, not `ms % 100 == 0` — with a collapsed backlog `ms` steps straight over the boundary and the whole
  ...
  if (out.wdt_kick) port_kick_grant(PORT_KICK_TOKENS); else kick_tokens = 0u;   /* E82: SysTick owns the cadence, the tick owns the permission */
  ...
  /* E82 (G-12): confirm the slot that is RUNNING, not whatever the record calls pending. They diverge on the path
     updater.c already handles — a store failure leaves flash saying "pending" while the confirmed slot runs — and the
     old test then promoted an image that had never executed, with the running image's minimum as its baseline. */
...
/* E82 (G-03): the window watchdog across start-up. The bootloader's last service is ≤ 5 ms old at the jump; the E81 form
   first serviced it from SysTick after APP_WDT_KICK_MS good ticks + the next 10 ms boundary ≈ 20 ms AFTER the interrupts
   started, behind clock, ADC calibration, the NVM mount and its software CRC — 32–53 ms in all against 23.375 ms, i.e. a
   reset at every application start. boot_kick() services it between the long start-up steps, never closer than 3 ms to
   the previous edge (the 2.22 ms early boundary), and main() hands SysTick a first purse so the cadence continues until
   the tick has proved both control interrupts alive and takes the permission over. */
...
int main(void) {
  ...
  vrefint_counts = adc_vrefint_read();                                /* E82 (C-03): ADC3_IN20 at 28.5 µs, before adc_init */
  ...
  b.uid = pmp_crc32((const uint8_t *)UID_BASE, 12u);                  /* E82 (G-18): all 96 bits — the first word alone is the wafer lot, shared by a whole reel; same identity as the bootloader's */
  b.vrefint_v = (VREFINT_CAL > 1000u && VREFINT_CAL < 2000u) ? (float)VREFINT_CAL * 3.3f / 4096.0f : 0.0f;   /* E82 (D-07): factory counts at 3.3 V */
  b.nvm_page_size = nvm_port_page_size();
  b.evlog_pages = (uint8_t)(FM_EVLOG_SIZE / nvm_port_page_size());
  /* E82 (M-20): SCB_VTOR now points into TCM — startup.c keeps the slot the bootloader entered in port_slot_vtor */
  ...
  fwdgt_start();
  boot_kick();
  port_kick_grant(PORT_KICK_TOKENS + 2u);      /* E82 (G-03): 50 ms of SysTick services while the tick earns its own permission */

  irq_prio(76u, 0u); irq_enable(76u);          /* HRTIMER faults */
  irq_prio(13u, 1u); irq_enable(13u);          /* E82 (M-21): DMA0 channel2 — ADC2 end of sequence, 100 kHz control */
  ...
  for (;;) {
    ...
    __asm volatile ("cpsid i");                 /* E82: tick_due -= n was a non-atomic RMW against SysTick (a lost ms) */
