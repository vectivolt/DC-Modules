/* svc.h — the VMP service space (identifier bit 24 = 1): firmware update and boot control. Portable C99, sans-IO. The
 ...
 * The application runs none of SELECT / INFO / ENTER / RESET, and the space is not reachable from a protocol profile.
 * The way INTO the bootloader is a sealed HANDOFF_ENTER: the VMP ENTER_BOOT action on the native profile, or both
 * panel buttons held for 3 s inside the first 10 s after power-up on any profile (hal/app.c panel()) — the only route
 * a TonHe-profile module has.
 *
