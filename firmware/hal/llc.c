...
/* Tolerance-worst ZVS boundary fn for nominal Q = 0, 0.05 … 1.0, then 1.5, 2.0 and Q → ∞ (the series tank alone at
...
/* The tank's own no-load FHA gain M(fn) = Ln·fn² / ((Ln+1)·fn² − 1) and its slope, used to publish how much
...
void llc_cfg_default(llc_cfg_t *c, uint16_t kw) {
  ...
  c->d_min = 0.08f;
  c->d_on = 0.12f;
  /* ZVS constants — calculations/llc/tanks.mjs TANKS[sku] (n, Lm, cs, par) and DIES["23m"].qoss800 */
  c->n = 2.0f;
  c->lm_h = (kw == 50u) ? 35.6e-6f : (kw == 40u) ? 43.5e-6f : 56.0e-6f;
  c->cs_f = (kw == 30u) ? 330.0e-12f : (kw == 40u) ? 680.0e-12f : 1000.0e-12f;   /* tanks.mjs cs 330 / 680 / 1000 pF */
  /* 371 nC is NOT the C3M0021120K's charge (its 99 µJ is E_oss at 1000 V and Q = 3E/V overstates a flat
...
/* Continuous instead of ceil-to-grid. The table is a conservative UPPER envelope, so the curve has to stay at or above
   the grid point ABOVE q (the ceil value) — interpolating between the two points below would dip under the true
   boundary in the table's concave region. Interpolating between ceil(q) and ceil(q)+1 keeps exactly the ceil form's
   conservatism while removing its up-to-0.11·fr ≈ 15 kHz steps, which would otherwise make f_min jump. */
...
/* The ZVS transition this operating point needs (llc.h). f is the frequency actually commanded, so the schedule
   follows PSM (f_max, the weakest magnetizing current) as well as PFM. */
#define LLC_ITOFF_PFM_K 1.0f   /* measured 0.85–1.30 × tank rms over 27 solved points; 1.2 falls up to 29 % short on time */
#define LLC_ITOFF_PSM_K 1.6f   /* the shifted leg turns off at the PSM tank peak (crest ≈ 1.6 × rms) */
static float zvs_dead_s(const llc_cfg_t *c, float f_hz, float v_bank, float v_bus, float i_comm) {
  ...
  float ic = fmaxf(im, (isfinite(i_comm) && i_comm > 0.0f) ? i_comm : 0.0f);   /* the leg's own commutation current */
...
/* The WEAK leg in phase shift needs a SHORT, valley-timed dead time — never the long I_m time (charge ÷ magnetizing
   current, up to 900 ns) that "commutates on I_m alone" would suggest. At load the zero state ends with the rectifier
   ...
   The switched model's dead-time sweep puts the best edge at 90–130 ns on the one-die leg with a 40 ns window, in CCM
   and DCM alike (90–190 ns in CCM and 190–270 ns in DCM on the two-die 50 kW leg), and holds the 30 kW PAR150 loss at
   9 W per die instead of 50 W. The valley time is the tank's and the leg's own: k · (π/2) · √(L_r · C_node),
   C_node = 2·par·(Q_oss(V)/V + C_s) — no measurement, nothing to tune per unit. k = 0.82 on the RFQ-bound charge puts
   125 / 186 / 189 ns (30 / 40 / 50 kW) inside those windows; T-58 (leg-node probe) is the bench row that trims it. */
...
void llc_step(llc_t *l, const llc_cfg_t *c, bool en, float u, float v_bank, float p_out, float v_bus) {
  ...
  /* how many volts of output one unit of demand is worth HERE, as a ratio to the sensitivity the shipped CV gains are
     tuned for. The map itself is exactly what the cycle-by-cycle plant validated; its sensitivity spans 23× over the
     150–1000 V envelope (53 V/unit near resonance against 1211 V/unit in deep phase shift), and ONE fixed PI pair
     driving that unnormalised crosses |T(z = −1)| = 1 at the phase-shift end and limit-cycles at 5 kHz, 0–160 V pk-pk,
     exactly at the cable-check/pre-charge point of every low-voltage pack. The caller divides its voltage-loop gain by
     this ratio (app.c), which holds the loop gain constant without moving a single validated operating point.
     PFM: dM/dfn · dfn/du · v_bus/n. PSM: the bridge's fundamental is (4/π)·v_bus·sin(π·duty/2), so the sensitivity
     carries that cosine — it is what makes deep phase shift so much livelier than resonance. */
  { float ln = llc_ln(c), vbn = fmaxf(v_bus, 1.0f) / c->n, kv;
    ...
    /* the ratio must follow the point the loop is REGULATING, not the instantaneous duty. Inside a burst the duty collapses
       ...
       while it is trying to climb back out — the drawn film bank would fall 55 V before the bridge restarted. A real
  ...
  /* Per-leg schedule: in PSM leg A commutates on I_m alone and leg B (the shifted leg) near the tank peak
     (crest ≈ 1.6 × rms); in PFM both legs turn off at the same current, ≈ 1.2 × rms above resonance.
     The rms comes from the operating point, not from a measurement. I_RES is converted at 100 kHz locked to the PFC
     carrier while the tank runs 77–203 kHz asynchronously, so a 10-sample mean of it reads ≈ 0 — the schedule would
     silently fall back to its long I_m value on every leg — except near 100/200 kHz, where the alias lands at DC and it
     reads an arbitrary value up to the crest. The tank rms is the reflected output current in quadrature with the
     triangular magnetizing current, and llc_step already holds every term: π/(2√2·n) · P/V_bank, and I_m/√3. */
  ...
  if (!(v_bank >= 100.0f)) { l->burst = false; l->gate = l->duty > 0.0f; }   /* a discharged output: continuous phase shift */
    ...
    /* the packet is bounded at the SETPOINT, not at the duty. Stopping the bridge purely on duty parks the output up to
       +2.9 % high with 4 % pk-pk burst ripple on the drawn 9.4–40.2 µF film bank (a 200 µF test plant hides it: the
       error scales as 1/C). With a reference in hand the bridge only stops once the node has reached it. */
    if (l->duty < c->d_min && (!(l->in_v_ref > 0.0f) || v_bank >= l->in_v_ref)) l->burst = true;
    /* the packet ENDS on the setpoint too, not on the duty alone. Restarting purely on the demand climbing back to d_on
       leaves the off time at the mercy of the voltage loop's gain, and once that gain is normalised for stability
       (k_norm above) the drawn film bank falls 21 % before the bridge comes back. A node 1 % under its reference is
       reason enough to switch, whatever the demand is doing, and it bounds the burst ripple on every SKU without
       depending on a tuned loop gain. */
