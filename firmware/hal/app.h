/* app.h — the module application: the one place the power core, the protocol profile and the portable HAL modules meet.
...
/* HRTIMER fault channels (umod-pinmap, GD32G553 UM Table 25-21) */
#define APP_FLT_IB     0u   /* CMP1 ← I_B0 */
#define APP_FLT_IA     1u   /* CMP3 ← I_A0 on PB0 */
...
/* relay-coil economizer duties (docs/firmware-guide.md, relay economization): 100 % pull-in for 60 ms after a close command, then 40 % hold
...
/* Fan supervision is a curve, not a floor — full-speed tach of the selected fan (two pulses per revolution); a fan
   below 35 % of the speed its duty commands for 3 s has failed. Calibrate at EVT with the chosen fan. */
...
typedef struct { float f_hz, duty, dead_s, dead_a_s, dead_b_s; bool gate; } app_llc_out_t;   /* per-leg ZVS transitions to program (dead_s = the longer, for single-value readers) */

typedef struct {
  float t_pfc, t_inlet, t_llc, t_xfmr;   /* NTC counts */
  float v24, v15, vrefint, avmid;        /* counts — AVMID is converted and checked */
  bool late;                             /* this tick is a collapsed backlog — the heartbeat re-baselines */
  uint16_t di;                           /* APP_DI_* */
  float tach_hz[4];
  uint8_t can_state;                     /* 0 error-active · 1 error-passive · 2 bus-off */
  uint16_t can_rx_ovr;                   /* drains that found every RX mailbox occupied — frames may have been lost */
...
typedef struct {
  ...
  float relay_duty[APP_RLY_COUNT];       /* economizer duty per coil (0 = open) */
...
typedef struct {
  ...
  float vrefint_v;                       /* the part's own factory VREFINT (V); 0 = not available */
  uint8_t reset_cause;                   /* RCU_RSTSCK bits 31:24 raw — the HAL classifies */
  uint8_t hw_rev;                        /* card revision strap (0 = rev A) */
  bool handoff_reboot;                   /* the last reset was a deliberate, sealed reboot */
  bool disch_pending;                    /* the pre-reset handoff says a discharge was commanded */
  uint32_t uid, fw, nvm_page_size;
  uint32_t fw_crc, boot_ver;             /* from the image header / bootloader (0 when absent) */
  uint8_t boot_state;                    /* VMP object 0x0009 (boot/bootctl.h state) */
  uint8_t evlog_pages;                   /* port pages 4 … 4 + n − 1 hold the event ring (0 = no event log); pages 2/3 are the bootloader's record store */
} app_boot_t;
...
typedef struct { bool en, ser; float v_ref, i_ref, v_scale, fold_hi_v, fold_crest_v; } app_llc_ref_t;   /* fold_crest_v = 1.414·VLL_max */
...
typedef struct {
  ...
  dielim_cfg_t die_cfg; dielim_t die; float iline_rms;   /* junction observer · worst phase rms for it */
  nvm_t nvm;
