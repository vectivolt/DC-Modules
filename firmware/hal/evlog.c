...
void evlog_add(evlog_t *l, uint8_t kind, uint8_t code, uint16_t arg, uint32_t t) {
  /* A storm is one event repeating. A chattering fault calling in on every tick would otherwise fill the queue with
     sixteen copies of itself, and each flush would write sixteen slots — moving the ring, and spending a page erase,
     sixteen times faster than the news in it justifies. An event already waiting in the queue is not added again; the
     flush still emits it once, so a genuine recurrence after the flush is logged normally and nothing is hidden. */
