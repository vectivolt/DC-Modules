 ...
 * E82: two closed forms. PFM (and the shifted leg B): charge ÷ the leg's commutation current, the larger of I_m and
 * k·I_rms with the rms from the operating point (M-31). Phase shift, WEAK leg A: k·(π/2)·√(L_r·C_node) (C-11 / F-H1-1). */
...
static double clamp_dt(double t) { return t > LLC_DT_MAX_S ? LLC_DT_MAX_S : (t < LLC_DT_MIN_S ? LLC_DT_MIN_S : t); }   /* E82 (M-15): the floor is the port's */
...
static void llc_checks(void) {
  ...
  for (int n = 0; n < 3; n++) {
    ...
    if (!(l.dead_a_s >= LLC_DT_MIN_S && l.dead_a_s < 250e-9f)) band = 0;   /* 125 / 186 / 189 ns — E81 parked this leg at 208–900 ns */
  ...
  ck("E82 LLC dead time: PS150 weak leg = 0.82·(π/2)·√(L_r·C_node), shifted leg and PFM legs = charge ÷ commutation current, all three SKUs to 1 ns", ok);
  ck("E82 LLC dead time: the phase-shift weak leg lands at 120–250 ns (E81 parked it at 208–900 ns and the node swung back to the rail)", band);
  ck("E82 LLC dead time: per-leg schedule — leg B never waits longer than leg A in phase shift, PFM legs are equal, nothing under the floor", legs);
  ck("E82 LLC dead time: a light PFM point still gets the long magnetizing-current time", light);
  /* E82 (M-15/T-12): the floor itself. The NSI66x1A is 70/80/110 ns of propagation delay with no matching spec, so the
     two drivers of a leg can differ by 40 ns and a 60 ns command cross-conducts. This row fails against the E81 value,
     and pins the port constant that has to track it: hrtimer.c DT_MIN = 52 steps of 2.3148148 ns on DTGCKDIV = 0010. */
  { int dt_ok = LLC_DT_MIN_S >= 120e-9f && LLC_DT_MIN_S < LLC_DT_MAX_S;
    ...
    ck("E82 LLC dead time: the floor is 120 ns (NSI66x1A 70/80/110 ns, unmatched) and hrtimer.c's 52 DTG steps land on it",
       dt_ok && floors); }
...
/* E82: ideal mirror contacts, as host_sim drives them — the checks below run past F.19's 100 ms window, where a
   hand-set relay_fb would latch F.19 instead of the row under test (every E81 check above ends sooner) */
...
static void fsm_checks(void) {
  /* F-A-11 → E82 (E-02): the bypass closes on a SETTLED link (two 20 ms steps < 3 V), never on a link that is still rising,
     and a flat-topped supply — rectified peak 0.955 of 1.414·V_rms, where the 0.97 rule parked the module into F.20 — closes */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 50u);
    ...
    ck("E82 E-02: the bypass closes on a settled link within 20 V of its final value on flat-topped mains (0.955 of the sinusoidal crest); a link parked at 80 % never closes and is F.20",
  ...
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 50u);
    ...
    ck("E81 F-A-7: an idle but charged link is still watched — the midpoint row fires with both stages stopped",
       idle && (f.latched == FC_MID_IMB || f.latched == FC_HALF_OV)); }   /* E82: 480 V on a half is F.38 (LATCH) and replaces F.06 */
  ...
  /* ---------------- E82: the rows this review added. Each fails against the E82 baseline (the lead's patch). */

  /* E-05 (F.18): a bypass pole that never releases. Precharge cannot see it — the settled-link rule leaves in 61 ms,
     under F.19's 100 ms window — so the commanded discharge, the one place the contact is open for seconds, is where
     the release is checked. The report waits for the end of the dump, so it can never abandon one (E-06). */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 50u);
    pmp_in_t in = fsm_base();
    ...
    ck("E82 E-05: a bypass whose mirror never releases is F.18 at the end of the discharge; a healthy one reaches OFF (F.18 was declared and never implemented)",
       dumping && f.latched == FC_WELD && g.st == ST_OFF && g.latched == FC_NONE); }

  /* E-06: ST_FAULT ends the dump (E80 HR-11), so any row that latches during a commanded discharge abandons it with
     the link charged — and F.35 fires on a SINGLE late tick, which is what a flash write during a shutdown produces. */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 50u);
    pmp_in_t in = fsm_base();
    in.vbus = 700.0f; in.vbank_a = in.vbank_b = 350.0f;
    pmp_fsm_resume_shutdown(&f);
    ...
    ck("E82 E-06: F.35, DESAT and an implausible sample during the commanded discharge leave the dump running — and it is still bounded",
       still && f.latched == FC_DISCH && !f.out.q_disch); }

  /* E-07: the E77 rows judge the RATED current only, so a failed CC loop may deliver 0.96 of rated into a 0.06
     request for ever. 500 ms and a 15 %-of-rated margin cannot be reached by any CC transient. */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 50u);   /* i_rated 166.7 A */
    ...
    fsm_steps(&f, &in, 120);
    ck("E82 E-07: 160 A delivered against a 10 A command latches F.15 at 500 ms, and a 400 ms excursion does not",
       quiet && patient && f.latched == FC_OUT_OC); }

  /* H2-2: F.01 at a line return. The comparator fires on the surge the boost diodes conduct when the line steps back
     over a link that sagged to its crest; as a LATCH row that made every utility dip a service call. */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 30u);
    ...
    ck("E82 H2-2: a line over-current inside 500 ms of a sag is filed as the grid's (F.08, AUTO_EXT, uncounted, bypass opened, self-clearing); on a quiet line it is still F.01 and still latches",
       quiet_latches && rides && grids && f.latched == FC_NONE && f.st != ST_FAULT); }

  /* E-08: F.05 on a grid interruption. The rms line values lag the link collapse by up to a cycle, so the row fires
     before its own cause is visible — and it is counted toward F.31, so five brown-outs locked the module. */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 50u);
    pmp_in_t in = fsm_base();
    f.st = ST_RUN; f.out.pfc_en = true; f.out.llc_en = true; f.out.v_max = PMP_PAR_VMAX_V; f.out.k_pre = true;
    in.enable_req = true;
    ...
    fsm_steps(&g, &in2, 300);
    ck("E82 E-08: a bus-UV the line explains within 200 ms is re-filed as the grid's and un-counted; one on a healthy line stays F.05, AUTO_INT and counted",
       busuv && regrade && g.latched == FC_BUS_UV && pmp_fault_class(g.latched) == FCL_AUTO_INT && g.counted == 1u); }

  /* E-09: F.26 was a code nothing ever latched, and neither INIT nor SAFE had a bound */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 50u);
    ...
    ck("E82 E-09: an aux that never comes up is F.26 after 5 s in INIT and in SAFE (it was an unbounded silent park), and the row clears itself when the rail returns",
       waiting && said && back && safe && g.latched == FC_AUX_UV); }

  /* E-10: the exit tested three channels on ONE sample — a glitched low reading ended the dump at 700 V */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 50u);
    pmp_in_t in = fsm_base();
    in.vbus = 700.0f; in.vbank_a = in.vbank_b = 350.0f; in.vout_meas = 30.0f;
    pmp_fsm_resume_shutdown(&f);
    ...
    ck("E82 E-10: a single glitched low sample does not end the discharge, and OFF waits for the output node the 450 kOhm bleeder drains",
       held && node && f.st == ST_OFF); }

  /* E-13: the matrix commands stayed asserted through an aux dropout, so the contacts re-made themselves the instant
     the rail returned — ahead of the SAFE hold and without the E76/R03 make-permit */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 50u);
    pmp_in_t in = fsm_base();
    ...
    in.aux_ok = true; in.enable_req = false; fsm_steps(&f, &in, 2); in.enable_req = true;   /* E76/R06 re-arm */
    fsm_steps(&f, &in, 700);
    ck("E82 E-13: an aux dropout opens the matrix commands with the coils, so the re-make goes through the permit (bleeders on, contacts open) instead of closing on hardware",
       opened && !f.out.k_para && !f.out.k_parb && f.out.q_disch_bk && f.latched == FC_NONE); }

  /* E-20: the comms timeout stops the module and writes no code anywhere — warn 0, fault 0, no reason on the panel */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_in_t in = fsm_base();
    in.vbus = 400.0f; in.can_age_ms = 5000u;
    pmp_fsm_step(&f, &in);
    ck("E82 E-20: a communication loss reports PMP_W_COMMS_LOST (F.28 is graceful, not silent)", (f.out.warn & PMP_W_COMMS_LOST) != 0u); }

  /* E82 (found here): prechg_ms was never reset with the state, and nothing resets it on an in-range line — so a CLEAR
     after F.20 re-entered precharge with the window already spent and re-latched F.20 on the next tick, for ever. */
  { pmp_fsm_t f; pmp_fsm_init(&f); pmp_fsm_set_rating_kw(&f, 50u);
    pmp_in_t in = fsm_base();
    ...
    fsm_steps(&f, &in, 500);
    ck("E82: a CLEAR after F.20 really re-precharges — the window is reset with the state, not left spent",
...
static void app_checks(void) {
  ...
  /* E82 (C-02) replaces E81 F-D-10 (O-15): the sign-following reference is GONE. Every comparator is non-inverting into an
     active-high fault input, so a reference below AVMID is a fault asserted in normal operation — the converter could not
     run. The references stay positive whatever the sign of the phase current; the negative half-cycle is covered by the
     100 kHz software magnitude trip (app_test) and by the other two phases' comparators (Σi = 0), which is also what
     makes the CT primary orientation immaterial. */
  { static app_t a; app_boot_t b; memset(&b, 0, sizeof b);
    ...
    ck("E82 C-02: the three F.01 references stay ABOVE AVMID whatever the sign of the phase current — a reference below it is a standing fault",
       a_hi && b_hi && c_hi); }

  /* E82 (M-08, firmware half): the 50 kW-air aux budget is ≈ 120 W on a 110 W stage and the NCP1252's over-current is
  ...
  { static app_t a; app_boot_t b; memset(&b, 0, sizeof b);
    ...
    ck("E82 M-08: no fan starts before 500 ms of in-spec aux rails, and the second fan PWM group starts 300 ms after the first",
...
/* ---------------------------------------------------------------- E82 (C-11 / M-06) the junction observer, hal/dielim.c */
...
static void dielim_checks(void) {
  ...
  ck("E82 C-11: the mainstream point (50 kW PAR400, 75 A tank, 77 °C base) carries no fold — the observer costs nothing where the design is sound",
  ...
  ck("E82 C-11: 50 kW in deep phase shift at 725 V (≈ 150 W per weak-leg die) is DECLINED inside 5 s and stays declined while the stage runs — no cool-deliver-heat cycle",
     declined && k == 1.0f && w > 100.0f);
  dielim_reset(&d);
  ck("E82 C-11: stopping the stage forgets a declined point — the next start begins unfolded", !d.declined && die_run(&c, &d, 0.0f, 60.0f, 0.0f, 60.0f, 10) == 0.0f);
  ...
  ck("E82 C-11: a 0.5 s start passage through deep phase shift (650 V link, 45 °C base) does not fold", k == 0.0f && !d.declined);
  ...
  ck("E82 C-11: the 30 kW at PAR150 / 725 V / hot settles on a PARTIAL current with the junction inside the fold band — a fold, not a refusal",
     !d.declined && k > 0.05f && k < 0.95f && d.tj_llc > DIELIM_TJ_C - DIELIM_BAND_K && d.tj_llc <= DIELIM_TJ_C);
  /* M-06: low line on a high link, hot — the Vienna die folds the 50 kW air to the grid's 86–93 % */
  dielim_cfg_default(&c, 50u, false); dielim_reset(&d);
  k = 0.0f;
  for (int n = 0; n < 30000; n++)
  ...
  ck("E82 M-06: 50 kW air at 285 VAC on an 830 V link, hot, folds to 80–97 % on the Vienna die (unfolded it reads 164 °C)",
     k > 0.03f && k < 0.20f && d.tj_pfc <= 150.0f);
  k = die_run(&c, &d, 0.0f, 77.0f, dielim_pfc_w(&c, 400.0f, 800.0f, 74.8f, true, 120.0f), 77.0f, 20000);
  ck("E82 M-06: at 400 VAC / 800 V the same die carries full power unfolded", k == 0.0f);
  /* broken inputs are "no information", never a stop */
  dielim_reset(&d);
  k = dielim_step(&d, &c, NAN, NAN, NAN, NAN, 1.0e-3f);
  ck("E82 C-11: non-finite inputs leave the fold at zero — the NTC ladder and the hardware trips stand behind this, it must not stop a healthy module", k == 0.0f && !d.declined);
  ...
  { pmp_ctl_cfg_t cc; pmp_ctl_t s; pmp_ctl_in_t ci; pmp_ctl_cfg_default(&cc, 50u); pmp_ctl_init(&s); memset(&ci, 0, sizeof ci);
    ...
    ck("E82 C-11: ctl multiplies availability by (1 − die_fold), reports it as a thermal derate, and a declined point is 0 A — zero-initialised input = no fold",
